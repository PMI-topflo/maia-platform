...
- **`owners.account_number` is the only safe identity key for CINC sync/matching — never `owners.unit_number`.** User direction, 2026-09-09 (real incident: MACO), confirmed via `owners` query — `unit_number` is just the bare trailing digit and is **not unique across an association**: for MACO, `unit_number = '1'` is shared by MCCA1, MCCB1, MCCC1, MCCD1, MCCE1, MCCF1 — six unrelated accounts across different building sections. `lib/cinc-sync.ts` used to fall back to a `unit_number`-keyed match when `account_number`/`cinc_property_id` didn't resolve, which silently cross-wired Hispania Entertainment LLC's CINC records onto unrelated MACO owners (Carlos Fleites, Daniel Fernandez, Richard Garcia...) — fixed by removing that fallback entirely. **When the user says "unit number" in a request that implies matching, identifying, or looking up an owner/account (not just displaying a label), confirm whether they mean `account_number` before writing code** — the two are conflated in casual conversation but are NOT interchangeable in this codebase.
- **Never `.maybeSingle()` a query against `owners` by unit/account — use `lib/owner-lookup.ts` (`findOwnerRows`/`findMergedOwner`) instead.** A co-owned unit has one row **per owner** (e.g. MANXI 103: Andre + Marcia Danford; MANXI 706: "1125 Digital LLC & Rodrigo Campos"), and `.maybeSingle()` throws `PGRST116` on multiple rows, which most callers swallow into a silent `null` (or, with `.limit(1)`, silently picks an arbitrary co-owner). `account_number` is exactly as vulnerable as `unit_number` here — it identifies the account, not the individual owner. Confirmed live in ~20 call sites, 2026-09-09 (PR #850), three of them live user-facing hard failures (the owner's own compliance portal, the tenant compliance page, the pre-apply "is this unit on file?" check) — this had already caused two prior real bugs before that full audit (the admin dashboard header; `lib/lease-packet.ts`'s lease-packet owner email) and a standing "grep before adding another" note that didn't stop it recurring. Do not write a fresh `.eq('account_number', ...).maybeSingle()` query against `owners` — import the shared helper.
- **`application_stakeholders.is_primary` is scoped per `role`, not per application** — "the primary owner," "the primary applicant," and "the primary agent" are three independent flags, by design (every "who is the primary X" read in this codebase filters `.eq('role', ...)` before `.eq('is_primary', true)`). Any code that inserts a new stakeholder into an application already in progress (e.g. `lib/preapply.ts`'s `addStakeholders()`) must mark it primary within its own role if that role doesn't already have one — never hardcode `is_primary: false`, and never assume "primary" is unique across the whole application (an agent-started application legitimately has an `is_primary=true` agent row AND, once added, an `is_primary=true` applicant row, simultaneously).
