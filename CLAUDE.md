...
- **Never `.maybeSingle()` a query against `owners` by unit/account** — a co-owned unit has one row **per owner** (e.g. MANXI 103: Andre + Marcia Danford; MANXI 706: "1125 Digital LLC & Rodrigo Campos"), and `.maybeSingle()` throws `PGRST116` on multiple rows, which most callers swallow into a silent `null`. This has caused two separate real bugs (the admin dashboard header's owner lookup; `lib/lease-packet.ts`'s lease-packet owner email, 2026-09-09 — a co-owned unit's lease packet was created with no owner email on file at all). Select all matching rows and join names/emails instead (see `app/api/admin/pre-apply/[id]/route.ts`'s owner query, or `lib/lease-packet.ts`'s `resolveUnitOwner()`).
