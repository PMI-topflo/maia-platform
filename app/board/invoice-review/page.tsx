// /board/invoice-review?token= — login-free board approval of an AP
// invoice. Public route (token-gated by the API). Server reads the
// token; the client fetches the invoice + handles approve / revision.
import InvoiceApprovalClient from './InvoiceApprovalClient'

export const dynamic = 'force-dynamic'
export const metadata = { title: 'Invoice approval — PMI Top Florida Properties' }

export default async function BoardInvoiceReviewPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px', fontFamily: 'Helvetica,Arial,sans-serif' }}>
      ...
      <InvoiceApprovalClient token={token} />
    </div>
  )
}
