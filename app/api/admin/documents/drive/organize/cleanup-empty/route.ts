...
export async function POST(req: Request) {
  ...
  const markUnits = body.markUnits !== false   // default: include the tag plan
  ...
  try {
    ...
    // Tag plan for the DIRECT child unit folders: empty ones get " - NO FILES
    // YET"; ones with files get it stripped. (Emptiness reflects the pending
    // deletions.) The client applies these renames (+ the deletes) in chunks so
    // it can show progress.
    const depth1 = [...nodes.values()].filter(n => n.depth === 1)
    const tagPlan: { id: string; newName: string }[] = []
    if (markUnits) {
      await Promise.all(depth1.map(async n => {
        try {
          ...
          if (isEmpty(n.id) && !tagged) tagPlan.push({ id: n.id, newName: `${stripNoFilesTag(name)}${NO_FILES_TAG}` })
          else if (!isEmpty(n.id) && tagged) tagPlan.push({ id: n.id, newName: stripNoFilesTag(name) })
        } catch { /* skip */ }
      }))
    }
    ...
    return NextResponse.json({
      ...
      deleteIds: candidates.map(c => c.id),
      tagPlan,
