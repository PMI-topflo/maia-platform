...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // NOTE: filing an approved document is a COMPLIANCE-RECORD action only. It
  // does NOT copy anything into the unit's Official Drive folder — application
  // documents (leases, renewals, purchase agreements) must stay in "On Going
  // Applications" until the BOARD approves, at which point the approval-move
  // engine promotes the whole application folder to Official. Staff upload
  // application docs from the application screen, which files them to On Going.
  return NextResponse.json({ ok: true })
}
