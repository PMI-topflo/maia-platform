...
PMI Top Florida Properties`
  ...
  try {
    await sendEmail({ to, cc: VENDOR_REQUEST_CC, subject, html, text: textBody })
  } catch (e) {
  ...
  await appendMessage(id, {
    ...
    body: `📤 Requested ${c.missing.join(' + ')} from ${c.vendor.vendorName ?? 'the vendor'} (cc ${VENDOR_REQUEST_CC}). Work order flagged — follow up if not received.\n\n${textBody}`,
  }).catch(() => null)

  return NextResponse.json({ ok: true, requested: c.missingKeys, to, cc: VENDOR_REQUEST_CC })
}
