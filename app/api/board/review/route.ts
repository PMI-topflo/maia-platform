...
async function assembleBoardPackage(application: Record<string, unknown>, applicationId: string) {
  ...
  const { data: subjectRows } = await supabaseAdmin
    ...
    .select('id, subject_index, name, status, report_url, report_data, completed_at')
    .eq('application_id', applicationId)
    .order('subject_index', { ascending: true });
  const subjects = (subjectRows ?? []).map(s => {
    const completedAt = s.completed_at as string | null
    return {
      id: String(s.id), name: s.name as string | null, status: s.status as string | null, report_url: s.report_url as string | null,
