...
export async function POST(req: NextRequest) {
  const { to, cc, bcc, replyTo, subject, body, html, persona, associationCode } = await req.json()
  ...
  try {
    const { messageId } = await sendEmail({ to, cc, bcc, replyTo, subject, html, text: body })
    void logEmail({
