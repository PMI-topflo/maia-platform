...
export async function POST(req: NextRequest) {
  ...
  const { data: subject, error: fetchErr } = await supabase.from('screening_subjects')
    .select('id, application_id, name, stakeholder_id, result').eq('checkr_order_id', event.orderId).maybeSingle()
  ...
  if (event.reportId) {
    try {
      await storeAndLinkReport({ id: subject.id, application_id: subject.application_id, name: (subject.name as string | null) ?? null, stakeholder_id: (subject.stakeholder_id as string | null) ?? null }, event.reportId)
    } catch (e) {
