...
// keyword matching. The classifier itself lives in lib/intent-classifier.ts
// (extracted so evals/run-intent-eval.ts can exercise it directly); this
// thin wrapper keeps the (ctx, message) call signature the voice + text
// handlers already use. ctx is unused by the classification — the message
// is the only input — but is kept for call-site symmetry.
async function classifyIntent(_ctx: CallerContext, message: string): Promise<{ intent: MaiaIntent; summary: string; confidence: 'high' | 'low'; restate: string }> {
  return classifyMessageIntent(message)
}
