...
Long session, three loosely-connected threads. **Note on the gap**: this doc's previous top entry is 2026-08-31, but real work shipped in between (PRs #739–#799 — the Checkr-first pipeline redesign phases, per-applicant vehicle/tax-returns declarations, the interview stage, Checkr key-mode fixes, digest/dashboard alarms, defaulting associations to Checkr, and more) without a handoff entry ever being written. That gap is NOT covered here — this entry only documents what this session actually did (PRs #800 onward), from git history and this conversation's own record. `docs/ROADMAP.md` has been kept current throughout as the safety net, so grep it before assuming something is undocumented.
