...
### Duplicate applicant rows merge instead of surviving Remove; signature reminders carry each signer's own link (PR, 2026-09-11)
...
- `sendSignatureReminder` used to send every board signer to `/admin/pre-apply/<id>` (the staff dashboard; user saw it on MANXI 706). Now one email per signer with their `/esign/<token>` link (falls back to `/board`, never `/admin`), plus a single office copy with the staff link and who was reminded. Both carry "Documents approved by: …" grouped by reviewer (Board / On-site manager / PMI staff) — 706 was approved entirely by staff, which the old email didn't reveal.
