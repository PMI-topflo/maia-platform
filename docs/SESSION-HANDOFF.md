...
- Fixes in `lib/staff-news.ts` + `lib/reconciliation-tickets.ts`: Opened/Resolved since the **previous edition** (`previousEditionStart`, previous weekday 5 AM ET; Monday covers Friday–Sunday) with a week-to-date line kept in the team header; Late = past due date or **idle** (updated_at) longer than the priority window; the 6 AM reconciliation cron **cancels** older still-open recon tickets as superseded (`supersedeOlderReconTickets`), and `POST /api/admin/reconciliation/supersede` ran it once by hand from the user's session.
