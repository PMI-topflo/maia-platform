...
### Shadow check rows — root cause traced and closed

Both reminder crons keyed the check row on `owner?.unitNumber || account`, so a run where the owner did not resolve minted a **second** row for the same unit keyed on the CINC account number — `MANXI710` beside `710` — with no owner name and no owner email.

**The root cause was already fixed on 2026-09-09 by PR #850.** Before it, the owner lookup `.maybeSingle()`d `owners` and threw PGRST116 on every CO-OWNED unit, swallowed into a silent `null` — and every affected unit here is co-owned (MANXI 710, 113, 103… two owner rows each). Proof in the data: **all 19 account-keyed rows were created in August, newest 2026-08-31, and not one has been created in the 15 days since.** `findMergedOwner` resolves all of them correctly today.
...
### Duplicate applicant rows merge instead of surviving Remove; signature reminders carry each signer's own link (PR #869, MERGED 2026-09-11 ~10:53 AM ET)
- MANXI 903 had "Shoodlyne Deus" twice (uploads on one row, payment on the other); Remove + Save left both cards because the delete either hit the `screening_subjects` FK (no ON DELETE) or would have orphaned her documents (FK SET NULL), and the route swallowed the result. `lib/stakeholder-merge.ts` now folds a dropped row into a kept row for the same person (name / email / phone): documents, screening subject, `na_items` scope keys and answers move over, then the duplicate is deleted. The route matches by name, then email, then phone, prefers the row with the most attached, and returns `warnings[]` the editor shows instead of silently re-showing the person.
- `sendSignatureReminder` used to send every board signer to `/admin/pre-apply/<id>` (the staff dashboard; user saw it on MANXI 706). Now one email per signer with their `/esign/<token>` link (falls back to `/board`, never `/admin`), plus a single office copy with the staff link and who was reminded. Both carry a **Document review** block — 706 was approved entirely by staff, which the old email didn't reveal.
...
### MANXI 1002 owner, second time: "already submitted" after the stale notice (2026-09-15)
- The stale notice sent the owner "Finish my application"; his link showed the done screen ("you don't need to do anything else") because HIS part was complete while the tenants owed six items. Now: the token GET returns `waitingOnOthers`, the done screen says "Your part is complete", lists what waits on the others and has **Add or replace a document** (reopens the checklist); strings in all 7 languages. Expiry notices are per person via `linesForRecipient` (still needed from you vs waiting on others; nothing of their own → FYI, no button; the unpaid notice only to applicants). Rule in memory [[preapply_submitted_not_a_gate]]: never one text for everyone on a resident-facing message.
- Dry run after the fix: 103's stale notice would clear (a document arrived after it) — the mechanism works.
- Same unit, third finding (#933): the staff "Request the missing documents" box prefilled only the lead applicant, so co-applicant Julian was never asked for his items. `tenantEmail` on the application GET and `directTenantEmails` in `request-docs` now take every applicant with an email (plus the tenant on file when different). Verified by the user.

### Lease-renewal expiry: violation warning + move-out notifications (2026-09-15)
- User (MANXI 1002, Rodao & Osorio): on the expiry date, owner and tenant get a **24-hour warning** that the renewal application expires and that staying constitutes a violation; on expiry the owner is told the unit will be vacant and the **board, on-site manager and staff** are told the tenant is moving out.
- `lib/application-auto-expiry.ts`: for `lease_renewal`, the run before the expiry (due within 30 h) sends the final warning to tenants (with link) and the owner (owner stakeholder, else co-owners on file via `findMergedOwner`), stamps `expiry_final_warned_at` and pins `expiry_due_at` 24 h later. On expiry: owner email + one email to each of `OFFICE_EMAILS`, board members, shared board mailbox and on-site managers. Digest shows "24-hour final warning sent". Migration `listing_applications_expiry_final_warning` applied in prod.
- Gotcha: the notice columns are read in a separate tolerant query; a missing column makes every notice look unsent (the dry run before the migration showed 103/1002/115 as "would notice" again). Apply the migration before the 6 AM cron runs.

### Automatic expiry of dead applications (2026-09-14, night)
- User: instead of manual housekeeping, email and expire — no files → 7 days; unpaid background check → 48 hours; paid but incomplete → after the 45-day screening validity; the daily email should list what expired and the files should move. Asked "who expires?" → **automatic**, provided Reopen can pull the Drive files back (it does now: `closed_drive` records the moved files; `reopenApplication` recreates the On Going folder and moves them back, tag stripped).
- `lib/application-auto-expiry.ts` + cron `application-auto-expiry` (10/11 UTC, acts 6 AM ET, before the 7 AM digest): rules `no_files` / `unpaid` / `stale` / `screening_expired` on applications still on the applicant's side; notice columns on `listing_applications` (migration `listing_applications_auto_expiry`); a notice clears when the person acts. Digest gets "Expired automatically" (Reopen link) and "On an expiry notice". Housekeeping page shows the notice chip.
- Legacy form list: unpaid rows older than 7 days and staff test rows hidden; a paid unlinked form always shows.
- First live run 2026-09-14: notices to 1003 (unpaid), 103, 1002, 115 (stale). User then: 1003 Rushayne Shaw, 303 Wilner Florestan and 603 Imad Mullah **paid Tenant Evaluation** → new housekeeping action `screened_elsewhere` ("Paid elsewhere" button on rows under an unpaid notice, #926) sets `screening_provider='tenant_evaluation'` and clears the notice; applied to all three. Unpaid legacy duplicates of a paid form now hidden at once (#927) — the legacy list is empty. Memory: [[application_auto_expiry]].

### Applications housekeeping: MAIA × Drive audit, 'expired' status (2026-09-14, evening)
- User asked how to audit MAIA × Drive × Tenant Evaluation for active vs dead applications, then: "Tenant Evaluation will be retired so we don't need to worry about it, let's fix MAIA and Drive." **Do not build anything on TE.**
- **Leasing → Housekeeping** (`/admin/applications-housekeeping`, `lib/application-housekeeping.ts`): open applications with idle days, documents, stage, lease on file + days since it ended, Drive folder present/missing, flags and **Mark expired**; Drive section with **duplicate folders → Merge**, **orphan folders → Move to Archive**, unmatched folders; **Reopen** on recently closed.
- **`expired`** = silent, reversible close for stale requests: `closeApplication(id, {mode})` in `lib/application-withdraw.ts` (withdraw kept as a wrapper), `reopenApplication`, `moveOngoingFolderToArchive` extracted; `expired` treated as closed wherever `withdrawn` was.
- Audit findings (MANXI): suggested expire 103, 703 (no documents; the applicant is the owner's LLC), 810, 901; 115 and the 1003 additional-occupant application are stale but reminded recently; two MANXI702 folders + "Unit 1003 - Rushayne K Shaw"/MANXI1003 duplicates; 511/605/703 without a folder. Memory: [[applications_housekeeping]].

### Processing audit + share link (2026-09-14, evening)
- User: realtors, applicants and board members complain applications take too long → six audit facts at the top of Communication history (previewed as an artifact with MANXI 303's real numbers, approved), then "a button to create a link for me to send by email to anyone".
- `lib/application-audit.ts` `buildProcessingAudit()`; `components/ProcessingAuditCard.tsx` shared by the staff block (`ProcessingAuditBlock`, with per-file detail) and the public page `/application-audit/[token]` (no emails, no file names). Share links in `application_audit_links` (id = token, view count, `revoked_at`); `GET|POST /api/admin/pre-apply/[id]/audit`; block has Create link (copies), Copy, Email it (mailto).
- Definitions: requests = document requests + staff "ask them to upload" replies + automatic reminders (not board reminders); approval speed = upload → first decision per file, **median first** (303: 33 min median vs 1.3 days average because one tax return took 12 days); missing = the reminders' outstanding summary; board = window opened / interview / letter signatures / letter emailed; bar = collecting vs board days.
- `listing_applications.screening_timeline` jsonb renders a "Background check · <provider>" row; seeded for MANXI 303 only with its Tenant Evaluation steps (user: "add only for this case"). Migrations `application_audit_links` + `listing_applications_screening_timeline` applied in prod from the user's session. Memory: [[application_processing_audit]].

### Shared board mailbox for applicant-facing emails (2026-09-14, late afternoon)
- User: the MANXI board does not want applicants to see their private emails; the board's shared address is **themanorsbuildingxi@gmail.com**; "all emails that copy the applicants and board will need to use this."
- `associations.board_contact_email` (migration `associations_board_contact_email`, seeds MANXI; applied in prod from the user's session). `lib/board-contact.ts` `boardCopyForApplicantEmail(code, exclude)` → mailbox on **CC**, active board members + on-site managers on **BCC**; without a mailbox the members stay on CC as before. Wired into the **interview introduction** (`requestInterview`) and the **invite with "CC board"** (`send-invite`). The signed approval letter was already BCC-only; board-review/signature emails go to the board alone and are unchanged.
- Staff edit it on `/admin/association-document-setup` → Association Questions ("Board mailbox"); red note when an interview is required and no mailbox is set. Forwarding from the mailbox to the members is configured in that Gmail, not in MAIA. Memory: [[board_contact_mailbox]].

### Lease reminder emails: one button per answer; occupancy answers were written under the wrong key (PR #915, 2026-09-14 afternoon)
- User on the MANXI 911 follow-up: "this button Tell us what's next is terrible — nobody understands, create something that makes an action to click" (the owner had replied by email instead). `lib/lease-renewal-email.ts` now renders one button per answer for owner (vacant / owner-occupied / tenant staying / lease already signed) and tenant (staying / moving out or moved out / lease signed); each opens the check-in page with the answer preselected and a single **Confirm** (never auto-submitted — link scanners). Confirmation text says what MAIA did and what comes next; intro distinguishes "has ended" from "about to end".
- **Bug found while testing:** "I pushed Vacant and it still shows Leased." `unit_occupancy` is keyed by **account_number** in every reader, but the lease-renewal check-in and the e-signed unit survey wrote the bare unit label — 10 invisible rows (MANXI 911 + 702 vacant, 514 + 401 owner-occupied, six survey "leased"). `setUnitOccupancy` now resolves the account (`accountNumberForUnit` in `lib/owner-lookup.ts`) and drops the label row; `POST /api/admin/units/occupancy-normalize` repaired the existing rows from the user's session. Memory: [[unit_occupancy_account_key]].

### Daily News made meaningful again (PR #912, 2026-09-14)
- User: "this daily report needs to be fixed, the data is not showing any valuable information." Monday's edition (5 AM ET) showed 0 opened / 0 resolved for everyone and 196 of 211 open tickets late.
- Causes: every cell counted "since Monday 00:00" (five hours old on a Monday); 146 of the 213 open tickets were "Daily bank reconciliation — <date>" rows (one per weekday for AP + AR, June → September) that nothing ever closed; "Late" was age > 7 days.
- Fixes in `lib/staff-news.ts` + `lib/reconciliation-tickets.ts`: Opened/Resolved since the **previous edition** (`previousEditionStart`, previous weekday 5 AM ET; Monday covers Friday–Sunday) with a week-to-date line kept in the team header; Late = past due date or **idle** (updated_at) longer than the priority window; the 6 AM reconciliation cron **cancels** older still-open recon tickets as superseded (`supersedeOlderReconTickets`), and `POST /api/admin/reconciliation/supersede` ran it once by hand from the user's session: **140 canceled** (70 AP, 70 AR). Team totals went 0 / 0 / 211 / 196 → 27 opened / 23 resolved / 73 open / 67 late; the corrected Monday edition was re-sent the same morning (user: "send the corrected edition now"). The remaining late items are real idle work — 34 of 37 open work orders (Paola + unassigned) and 33 of 36 open tickets.
...
- **705 data:** the car registration and driver's licence uploads were 0-byte Storage objects ("image is broken"); both rows deleted, `recordIntakeDoc` now refuses 0-byte objects and removes them (#907) — the applicant has to re-upload. GEICO card rescanned in prod after #908 deployed (user had already set 2027-07-01 by hand at 10:11 AM ET, before the deploy). Nothing was lost: the Drive copies of the registration and licence are 0 bytes too — both were in-browser iPhone camera captures. #910 adds a `file.size === 0` guard with advice (Camera app, then Photos) to the applicant link and the request link.
...
- Page `/admin/payment-reconciliation` (`lib/payment-reconciliation.ts`): payments table (gross / Stripe fee / net / deposit / screenings / Checkr / margin), deposits panel with **Received in bank** (`stripe_payout_receipts`) and a "What is in this deposit" breakdown, Checkr receipts upload (ZIP/PDF; `checkr_receipts` keyed by order id → re-uploads refresh, never duplicate) and a receipts list oldest-first with a **Debited** tick for the bank's "CHECKR TENANT" lines (`bank_debited_at/by`), exceptions list. Migrations `application_payment_reconciliation`, `checkr_receipts_bank_debit` applied in prod.
- Gotchas: pdf-parse returns empty text for Checkr receipts → `pdfTextViaPdfjs()` + `outputFileTracingIncludes` for the route (worker file); order id also read from the file name.
- **September 2026 reconciled live:** $750 gross / $22.95 fees / $727.05 net; Checkr $139.96 (4 receipts matched); margin $441.74. Payout `po_1UEIBf…` **$427.50 arrived Sep 11** = 706 $291.00 + 912 $145.35 + two refunded PMI test charges (Stripe kept $4.20 + $4.65). 702 and 903 nets ($145.35 each) await the next payout.
- **Bug surfaced by the tab, fixed #902:** the Stripe webhook skipped the Checkr order when the applicant came through `/apply` directly (no pre-apply bridge → provider resolved to tenant_evaluation). MANXI 702 (Ashlee Muthra) paid Sep 9 with nothing ordered; on 2026-09-13 I bridged the listing (`/api/apply/link-listing`) and ordered the screening (`awaiting_applicant`). The webhook now falls back to the application row's own provider.

### Withdraw an application — silent (PR #898, MERGED + deployed 2026-09-13)
- MANXI 411: agent Jorge Bavarese asked to cancel (applicant Maureen Garrick cannot move in). MAIA had no cancel action; the list's Delete only removes empty shells and would have refused (2 stakeholders). Built `lib/application-withdraw.ts` + `POST /api/admin/pre-apply/[id]/withdraw` + "⏏ Withdraw" on list rows and the application header: status `withdrawn` with `withdrawn_at/by/reason` (migration `listing_applications_withdrawn`, applied in prod), pending e-signs + the unit's lease packet voided, On Going Drive files moved to OLD/Archive tagged `_WITHDRAWN`, everything else kept. Applicant links show "This application was withdrawn". **No email is sent — user: "keep it silent, I will reply to Jorge myself."** `withdrawn` is a decided status: grey "Withdrawn" chip under Approved & closed.
- 411 withdrawn 2026-09-13 through the user's session: 1 lease packet voided, reason on record. It had no documents and no On Going folder, so nothing moved in Drive; the unit's Official folder `MANXI411 - 4174 Inverrary Drive` (and an older one under another root) are unit records, untouched. Withdraw only ever moves the application's own On Going folder.

### CINC sync: tenant/management address rows ignored; non-billable badge made sticky (PRs #895, #896, 2026-09-13 evening)
- BHB 5664 proposed the TENANT (a "Tenant Mailings" address row, AddressTypeId 4) as a third owner once #892 read every row. `isOwnerAddressRow()` keeps only Property Address (0), Owner's Offsite Address (2), Additional Owners (7); Management Company (8) and tenant rows are skipped. Portfolio had 1 tenant row and 2 management rows.
- LCLUB "LVUnits" (CINC status "Developer - NonBillable") showed ✓ SYNCED: pooling emails turned it from UPDATE into MATCH, and the homeownerLookup only ran for insert/update rows. Now every CINC-backed row is checked and the status is cached in `owners.cinc_homeowner_status` / `cinc_status_checked_at` (migration `owners_cinc_homeowner_status`, applied in prod 2026-09-13 via the admin runner). Verified: LCLUB preview → LVUnits `non_billable`.
- Also #894: tick marks are rebuilt from the fresh preview after Apply (the button used to keep the old count until a page reload).

### CINC sync reads names from BOTH address rows; partial-name matching (PR, 2026-09-13, later)
- User confirmed on ONE 402/603/702 (Alfredo Fantoni is the principal of Tross One LLC), ONE 502 (Marelys Triana, manager — CINC has no manager field, kept) and PVV 2459 (both owners + their LLC) that MAIA's "leftover" rows were right. CINC's two address rows carry DIFFERENT name pairs (603: owner row "Tross One LLC + Anthony Franco", property row "Alfredo Fantoni + Tross One LLC"); reading one row can never see everyone. `snapshotsFromCincProperty` now collects every distinct pair across all rows (owner rows first; up to 4 slots), collapses shorter/longer spellings of the same person (`namesCompatible`: token subset, ≥2 tokens), unions emails across rows, and tags pairs seen only on the property row (`source_row='property'` → `OwnerComparison.from_property_row`, badge "property row only", NOT pre-selected as INSERT). Matching gained PASS 1b (compatible names within the same PID/account).
- Archived by the user's decision today: ONE 601 Alfredo Fantoni (#857, previous owner), WBPA 470K JMS Trust (#23, previous owner) — plus BHB 5692 Rolando Lequeux (#85), ESSI 19 Interamerican Inc (#167), LFA 05 Vivian Noll (#281) earlier.
- Portfolio re-run after the change: flagged leftovers 5 (ESSI 13 Richard P Anderson; FIFTH 102 Beverly Pommells; MACO E9 Bernardo Cadavid; PVV 2477 Alexander Melnitsky; PVV 2485 Mouna Bouchoutrouch — new because emails are now the union), inserts 32 of which 4 property-row-only (BHB 5664 Margaret Adelman; LCLUB LP Esteban SaenzGutierrez; LFA 05 Vivian Noll — the deceased mother, do NOT insert; LFA 05 Debbie Piazza — same person as owner-row Debra Piazza).

### CINC sync read owner names from the wrong address row — second owners missed (PR, 2026-09-13)
- User check on MANXI 1001: CINC shows Owner 1 Marie Line Caroupin AND Owner 2 JERRY & LORANG GERMAIN, yet the sync flagged MAIA's Germain row as a leftover. `snapshotsFromCincProperty` took names from the property-address row (`OwnerAddress=false`: "" / "Marie Caroupin", no second owner) instead of the owner/mailing row (`OwnerAddress=true`: both owners, all 3 emails — the row CINC's own form shows). Same for LFA 03 (Suzanne Martin). Fix: pick the address row with the most populated name fields, tie → owner row. Re-run across all 26 associations: flagged leftovers 13 → 10 (MANXI 1001/1011/1013, LFA 03, GVH 1243, GK7 6845, SP B3 are real second owners now matched); a handful of new INSERT/name-UPDATE proposals appear on the Hubs (MANXI 3+3, ONE 6+3, LFA 1+1, PVV 1+1, SP 1, GK7/FIFTH/MACO/VPC5 1 name each) — staff review them there.
- Remaining flagged (user reviews on each Hub): BHB 5692 Rolando Lequeux; ESSI 19 Interamerican Inc (true duplicate of synced row 846); LFA 05 Vivian Noll (CINC now lists Beverly Kark); ONE 402/601/603/702 Alfredo Fantoni (principal behind Tross One LLC) and ONE 502 Marelys Triana (manager); PVV 2459 Dino Rocco (behind MADDOM LLC); WBPA 470K JMS TRUST. Hint copy softened accordingly ("older spelling, prior owner, or contact person filed as an owner").
...
- Data (via the user's session): MANXI `lease_renewal` / `background_credit` row set `active=false` (id `a929e747…`), and a `checklist.lease_renewal.background_credit = off` decision recorded on MANXI's questionnaire as existing configuration (id `50851560…`; shows as "pending adoption" because the decisions route records without applying — the live row is already off, and adoption re-applies it idempotently).
- Code: `lib/lease-renewal-rule.ts` — `ruleOnRenewal(code, unit)`: previous lease end from `unit_tenant_contacts.lease_end` (by CINC account) else the last approved application's signed-lease expiration; > `RENEWAL_GRACE_DAYS` (30) after it → the application is opened as **`lease`** (full checklist, screening, fee) with the reason on `review_note` and in the start response (`applicationType`, `notice`). Applied in `/api/pre-apply/start`, `app/api/admin/pre-apply/create`, and the check-in's `ensureRenewalApplication`. No lease end on file → renewal taken at face value.
- **Correction:** the five open MANXI renewals (1002, 901, 115, 407, 810) were ALL requested more than 30 days after their previous lease ended (1002: ended 2026-06-30, requested 2026-08-09 = 40 days; 810: 61 days; 407: 141 days; 901/115: previous leases ended spring 2026, requested Aug = ~80–100 days; the tenant records for 901/115 still carried 2025 dates — `previousLeaseEnd` now takes the LATER of the tenant record and the last approved lease). Under the rule they would need screening + fee as new leases. **User decision (2026-09-12): grandfathered — the rule applies from today forward; the open renewals (1002, 901, 115, 407, 507, 705, 810, 802) stay renewals without screening.** Their renewal checklists no longer carry background checks.

### Residents locked out of "submitted" applications; reminders now per person (PR, 2026-09-12)
- MANXI 1002's owner (Dieufait Raphael) wrote in: answering the vehicle / pet questions or signing returned "already been submitted". Cause: `declare`, `upload-url` and `record-doc` under `/api/pre-apply/[token]` refused any change once `listing_applications.submitted_at` was set — and five MANXI lease renewals (1002, 901, 115, 407, 810) were stamped submitted on the day they were created (Aug 9–19) while still missing 5–7 required documents, so the 3-day reminder kept asking for things the link would not accept. New `intakeClosed()` in `lib/preapply.ts`: only approved / declined / withdrawn / cancelled / archived / void block changes; submitted and under_review still accept documents and answers (that IS the live workflow — per-document review as items arrive). No data change needed.
- Second problem in the same email: the reminder listed the TENANTS' items (background checks, licenses, rules ack, the yes/no questions) to the OWNER as if they were his. `linesForRecipient()` in `lib/application-reminder.ts` now splits "Still needed from you" (applicants: own per-applicant + shared applicant items and the questions; owner: landlord items; agent: agent items) from "Still needed from the applicant(s)" (grey, no button, "each of them has their own link"). A person with nothing in either list gets no email.

### Applications rollout to all 25 associations — step 1 built: "What they have today" (PRs #877, #879, MERGED 2026-09-11 ~12:40 PM ET)
- **User plan (2026-09-11):** put all 25 associations live on the applications pipeline with exactly their CURRENT requirements first (no board meeting), then build the full board assessment ("what they have today vs what they can control with MAIA"). Start with Venetian Park I. Files: PDF always; most are already on each Hub's Documents page (22/25 have the Application Procedures package; all 25 have governing docs; ~10 files are image scans with no text).
- **Missing files the user is uploading** (Hub → Documents, category shown): Application Forms — TROP, LCLUB; Rules & Regulations — KANE, KGA, LCLUB, MACO, SHORE, SP, WBP, WBPA; Condo Docs — BHB.
- Built: `lib/onboarding-extraction.ts` (Sonnet reads application package + Rules + Declaration/By-Laws; scans transcribed once with Haiku in 30-page chunks, ≤150 pages, saved back to `extracted_text`; proposes every applications-scope catalog item + checklist cell with quote / source / confidence / rationale, plus "extras" with no slot yet), `lib/onboarding-proposals.ts` + migration `20260911_association_onboarding_proposals.sql` (extractions + proposals tables), routes `POST …/onboarding/[code]/extract`, `GET|POST …/proposals`, and `TodayPanel.tsx` as the questionnaire's first panel. Accepting = `existing_config` decision under the staff name, applied immediately.
- **⚠️ Correction to the morning entry: the Sep 9 onboarding tables had NEVER been created in production.** The `decision_window_days` ALTER was run by hand, and a `head:true` count query returned "ok/null" for a missing table (PGRST205 only shows on a real select). Found when the proposals migration's FK failed. Both migrations applied 2026-09-11 ~12:50 PM ET from the admin runner (`POST /api/admin/migrations/apply`, keys `association_onboarding` then `association_onboarding_proposals`). **Lesson: verify a table with `.select('*').limit(1)`, never a head count.**
- **Run live for VPCI** (12:54 PM ET): 22 proposals (20 high / 2 medium), 12 extras, 5 documents. 10 differ from what MAIA has: interview required for leases AND purchases (MAIA: off); every rule should be **Block** (MAIA: Warn); rental cap 10% in the operative clause vs 20% in an appendix (MAIA: 20%, and the Rules-Ack text says 20% — user to decide); "For Sale" signs are allowed (one, size-limited) but MAIA blocks them; decision window 10 days + 2 officer signatures not set in board config. **Nothing accepted yet — the user reviews on the page** (`/admin/cinc-sync/VPCI/onboarding`).
- Preview artifact reviewed by the user before wiring: https://claude.ai/code/artifact/ead2e5ca-4306-49ce-9811-5598f95dc328 (per [[feedback_preview_before_build]]).
- Still to build for the full questionnaire: board-facing assessment page (today vs MAIA-controlled, OTP, adoption), governing-docs → proposals for the rules-ack text, R&R redraft, compliance/operations/residents sections; catalog slots for fees / notice periods / forms without a doc key (today they land in "extras").

### Application Guide for Venetian Park I (PR #878, MERGED 2026-09-11)
- `lib/vpci-application-guide.ts` registered in `GUIDE_CONTENT` → live on all four delivery paths (`/apply/vpci/guide`, invite attachment, `@maia guide`, WhatsApp link). §2/§3 sub-headings are now per association (`processDek` / `checklistDek`; VPCI interviews everyone, MANXI purchasers only). Rendered and read: 3 pages.
- **Data nit for the user:** VPCI's `no_trust_purchase` rule label carries an internal note ("Board-confirmed 2026-08-25, clarifying…") that prints in the guide — shorten it in Association Setup or with SQL.

### Board reminder for 706 was wrong twice; reminder/card now roster-checked, "final approval" variant, MAIA mark, whole days (PRs #871–#874, MERGED 2026-09-11 late morning ET)
- Unit 706's 7 AM "Still needs your signature" claimed a letter was waiting — **no letter existed**: every document was AI pre-audited by staff and the board had never given the final approval on the card (the letter is generated only after that). It also went to Jorge Manzano (removed) because `document_review_rounds.recipients` is a snapshot.
- `sendSignatureReminder` (#871): filters stored recipients through `approversFor()` at send time; **variant `'approve'`** (subject "Waiting for your approval — Unit N", one button → `/board-review/<round token>`, copy explaining the letter follows the final approval) when no non-void `board_decision` esign row exists, `'sign'` otherwise; office copy states the variant. New staff route `POST /api/admin/pre-apply/[id]/board-review/remind` + **🔔 Remind the board now** button (has a `confirm()`; Claude-in-Chrome's click was cancelled at the prompt, so the send was done with a `fetch` from the logged-in page — user pressing it themselves is fine).
- Board card (#872, #873): GET reconciles the round's recipients with the active roster on every load (drop ex-members, refresh names/titles, add new, re-key `reviewer_verifications`), persisted so OTP/decide see the same list; "AI Pre-Audited by MAIA" line shows the MAIA mark (`display:inline-block` needed — Tailwind preflight makes `img` block).
- Days-left (#874): `Math.floor`, whole days only — the 11:52 AM send still read "30 days left" (sent before the merge; 29 d 6 h remaining). Window opened Sep 10 6:04 PM ET, due Oct 10 6:04 PM ET; resends never restart it.
- **Sent 2026-09-11 11:52 AM ET** (user's "yes, send it"): corrected reminder to Walter Giles, Karen Fisher, Angelique Philips, Kaye Brunson; office copy to PMI + Jonathan. Next step is the board's: open card → verify → Approve → letter generated → signature reminders switch to "sign".
- Cosmetic, user-side: roster names come from CINC as-is ("Walter Leslie Joseph, IV Giles", "KAREN FISHER JOHAN PISTANA") — tidy in Board Setup.

### Stripe processing fee pass-through — ⛔ declined (user, 2026-09-11)
User asked how to make applicants pay the $4.65 Stripe fee. Answered: Stripe never adds its fee to the customer's price; MAIA would have to add a gross-up line ($154.79 → net $150); Florida condo statute 718.112(2)(i) caps transfer/application fees at $150 per applicant and manager fees count, so a surcharge breaches it for MANXI and other condos; ACH (0.8%, $1.20) only lowers the absorbed cost and needs a webhook hold on the Checkr link until `checkout.session.async_payment_succeeded`. **User: "forget it, let's keep as it is."** Checkout unchanged (`app/api/create-checkout-session/route.ts`). Don't re-propose.

### Onboarding questionnaire migration — fully applied (2026-09-11)
The two `association_onboarding_*` tables were already present; `board_approval_config.decision_window_days` was missing. User ran the ALTER from the Supabase SQL editor; verified live (`column present: true`). Still not exercised end-to-end by staff.

### Prompt cache was silently broken in both cached call sites (PR #853, same evening)
Anthropic Console emailed "your prompt cache hit rate is low (up to 68% of direct API spend)". Audit of all 23 Claude call sites: only two cache anything — the freeform email reply in `lib/maia-command-processor.ts` (Sonnet) and the chat widget (`app/api/chat/route.ts`). **Both put `buildOfficeHoursBlock()` inside the cached system text, and that block embeds `Current time: <minute>`** — so the cached prefix changed every minute and nearly every call paid the 1.25× cache-write premium with no reads. Fixed by moving the block after the breakpoint as its own uncached system block in both. Added `logClaudeUsage()` to `lib/anthropic-guard.ts` (one `[claude-usage]` JSON log line per call with cache_read / cache_write / input / output / hit %) wired at both cached sites — before this nothing recorded cache fields, so the hit rate was unmeasurable. The other 21 sites are Haiku one-shot document readers (unique document each time; Haiku 4.5 also needs a 4,096-token prefix to cache at all) — no caching possible there; the Console's 68% is a generic estimate. Still open: the chat prompt bakes per-association FAQ/docs/language into the cached block, so hits only happen for the same association+persona+language within 5 min — a stable-prefix restructure would widen that. Twilio "WhatsApp Marketing Max Price" email reviewed the same evening: applies to Marketing templates only; MAIA's three templates are Authentication + Utility → nothing to do.

### Fewer, clearer staff emails about applications (PR, 2026-09-10)
User (with a Gmail screenshot of four "Additional document uploaded — … · PMI-A41E9A7C — …" mails plus a "[New Application] … · PMI-A41E9A7C"): "fix all these emails with this code that I can't understand, always send me with the association, account unit and link to the application, don't need to send an email for each document uploaded, let's leave in the daily email with the applications that I need to review, let's reduce the number of emails sent."
- **Removed four per-document staff emails**: legacy `/api/apply/documents/[id]` ("Additional document uploaded"), `/api/units/pre-apply/[id]/upload` ("Application document uploaded"), `/api/request/[token]/upload` ("Requested document uploaded"), and the first-document "📥 Documents arriving" in `/api/pre-apply/[token]/record-doc`. `UNIT_UPLOAD_NOTIFY_EMAILS` is now unused (noted in `.env.example`).
- **Daily digest** (`lib/application-review-digest.ts`, 7 AM ET) gained "New documents in the last 24 hours": every open pipeline application that received an `application_documents` row since yesterday, with the document labels, grouped by association → unit, linking to `/admin/pre-apply/[id]`; plus a "legacy form applications" block for old-form uploads (`applications.supplemental_documents`) not bridged to a pipeline application. New-document days now count as actionable, so the digest sends on them.
...
### Checkr report must land on each applicant's card automatically; "File as document" removed (PR, 2026-09-10)
User (MANXI 706, Quentin Smith + Tim Walker, both reports complete 9:33 AM ET): "the file as a document button is not working and Maia is not populating the background check ... make it automatically applying for each applicant the background check to their card ... the file as document does not need to exist anymore."
- Root cause (from code — Vercel logs timed out all morning): the Checkr subject names come from the payment form ("Tim Walker", "Quentin Smith") while the applicants' rows carry full legal names ("Timothy Dean Walker", "Quentin Jamal Smith"). `matchStakeholders` only did exact-name + equal-length pairing, and `fileReportAsDocument`'s own fallback was exact-name only, so the report was filed with `stakeholder_id = NULL` — invisible on a per-applicant checklist item, which is exactly what "not working" looked like. Two `.maybeSingle()` lookups on `listing_applications` by `detailed_application_id` were a second silent-failure path for a unit with more than one application row.
- **`lib/screening/stakeholder-match.ts`**: passes are now email → exact name → loose name (same last name, first-name prefix either way) → equal-length ordered pairing → last-one-left.
- **`lib/screening/report-storage.ts`** `fileReportAsDocument`: no `.maybeSingle()` (newest open bridged application wins); resolves the applicant with the matcher, excluding applicants already owned by sibling subjects; persists the resolved `stakeholder_id` back onto `screening_subjects`; replaces prior Checkr-filed copies as a list; returns `{ filed, stakeholderId, stakeholderName, warning }`.
...
### Upload a board-signed approval letter; staff sessions finally carry a name (PR, 2026-09-10)
- **MANXI 801 investigation** (user: "board approved in August and signed the letter through Maia but I can't find it"): no signed letter exists anywhere. MAIA's letter (created Aug 11, 11:33 PM ET; invites logged to Walter Giles + Angelique Philips) is 0/2 signed; no reminder rounds (automation postdates it); Drive's Official folder holds only the Dec 2024 approval; the On Going folder holds the Aug 10 applicant paperwork never filed onto the checklist. Told the user plainly.
- **"Can't find where to upload the approval letter"** — the Board Approval Letter checklist row was retired for new leases (Aug 21) and the Board Decision box only created/e-signed. New `POST /api/admin/pre-apply/[id]/signed-letter` + "Board signed outside MAIA?" upload control in the Board Decision box (signed-URL PUT via `/upload-url` with doc_key `board_approval_letter`, per [[signed_url_upload_gotcha]]): files the PDF, voids unsigned MAIA letters for the application/unit, runs `runBoardApprove(..., { signedLetterUploaded: true })` (new opt that bypasses the e-sign "fully signed" gate) + `handoffOnApproval`, optional distribution to all parties via the existing distributor with the uploaded PDF.
- **Blank staff names** ("approved a document — Unit 801" by nobody; letters "created by staff:"): staff sessions never had `displayName`/`contactName` set at login (only the email in `userId`). `verify-otp` now resolves the name from the staff directory; new `staffLabel(session)` in `lib/staff-auth.ts` (name → email → 'staff') used by review-decision, decision-page, signed-letter, and the onboarding routes. Existing sessions get the email until they log in again.

### MANXI 608 on the expired-leases list, and board emails reaching inactive members (PR, 2026-09-10)
- **Angelique Philips (board President) asked why her unit 608 is on the weekly "Expired leases" digest.** Traced: the digest reads `unit_tenant_contacts`; 608's row was written by the Aug 3 Drive approval-letter import ("approval move") from a **Certificate of Approval for Additional Resident** (Jun 26, 2025 — Sean Reid, her fiancé, added to her own unit). The import dropped her name (owner), kept him as "tenant", invented `lease_end` = approval + 1 year (Jun 26, 2026) and marked the unit `leased`. Same pattern on 511 (Mar 2025 additional-resident letter → invented Mar 17, 2026 end; that unit IS leased, renewal application open).
- **Code fix** (`app/api/admin/documents/drive/organize/approval-move/route.ts`): an `additional` letter never gets `plusOneYear`; owner named on the letter → occupancy `owner_occupied`, no tenant record, no approval-letter expiry; otherwise merge the name onto the existing tenant record and keep its dates.
- **Data fix NOT applied from here** (the permission classifier blocked the write script): SQL handed to the user for the Supabase editor — delete 608's tenant row, set 608 occupancy `owner_occupied`, delete 608's and 511's `lease_renewal_checks` rows, null 511's invented `lease_end`.
- **"MAIA is sending for board review to board members that are now inactive"** (706 round, Sep 10 6:04 PM ET, went to Jorge Manzano): `lib/board-review-email.ts` `approversFor()` read the OLD `board_members` table (unmaintained: stale titles, Jorge still active). Now reads `association_board_members` (active, substitute-aware). Also fixed: `lib/report-feedback.ts` and the monthly board-message route merged/fell back to the legacy table even when the real roster exists (now legacy only if the roster is empty); the SMS/voice webhook's board context + bookkeeping maintenance-request email now use the roster. **Still on the legacy table (login/identity, not touched):** `app/board/page.tsx`, `app/api/association-lookup`, `app/api/webhook` role detection (2 spots), `lib/reports/association-stats.ts`, `add-person`/`pending-approvals` inserts — an inactive member may still be able to log in as board; separate follow-up.

### Board identity moved to the roster (PR, 2026-09-10 — follow-up to #863)
User: "fix it anyway" (they'll also Remove ex-members on the Board Members page). New `lib/board-roster.ts`: `activeBoardByEmail` (roster, active, substitute-aware), `activeBoardByPhone`/`activeBoardByPhoneFragment` (legacy phone book → accepted ONLY when the same email is active on the roster), `addBoardMember` (roster row + legacy row for the phone). Switched: `app/api/association-lookup` (board login role → roster id; `/board` page already handled uuid ids), the SMS/voice webhook's three phone→board lookups, `lib/reports/association-stats.ts`, and the `add-person` / `pending-approvals` board inserts. Verified live: Jorge Manzano (inactive) rejected by email and by phone; Walter/Angelique found. The legacy `board_members` table is now only a phone book. SQL for 608/511 data fix was run by the user ("Success. No rows returned").

### Board card balance banner never omitted (#865); purchase letter shows advance maintenance (#866); Checkr link is automatic
- **#865** — "board keeps asking me if the unit is current": the Sep 8 balance banner vanished on MANXI 409 because `ownerBalanceInfo()` returned null when CINC's payment-status call had no balance. Now the ledger link is minted as soon as the account resolves, the balance falls back to the last ledger line, and the banner has three states (current $0.00 green / balance due red / "could not be read" gray, link kept).
- **#866** — board wants the approval letter to show the pre-paid assessments by credit score. New `lib/advance-maintenance.ts`: rule-driven (`credit_score_advance_maintenance` bands + `international_no_credit_advance_maintenance`), inputs = staff-entered `application_stakeholders.credit_score` on the primary applicant (or the international declaration) + the unit's latest quarterly assessment from the CINC ledger (`currentQuarterlyAssessment`, now exported from `lib/application-esign-forms.ts`). Purchase letters only; row "Advance maintenance — N quarterly assessments (M months) — $total" + sentence. **User wording: paid in advance AT closing; the title company holds the funds and remits per the estoppel letter** (not "prior to closing"). Previewed via the real renderer with MANXI 303 (692 → 6 months → 2 × $3,398.33) before creating.
- **Answered:** after the applicant pays, the Stripe webhook triggers the Checkr orders itself and Checkr emails each applicant the consent link — no button needed; all 23 open MANXI applications carry `screening_provider = maia_checkr`. The blue button is for unpaid (sends the pay link), expired re-screen, or a failed order.
- Also this evening: MANXI unit-audit proposal PDF for the board (2 pages, from live data) — not in the repo, sent to the user.
### MANXI 603 "important notice" was correct; owner notice now states amount + date (PR #860)
Agent Harris Chawla replied confused to MAIA's Sep 8 "Application on Unit 603 — important notice"; user saw $0 in CINC. Live ledger pull (CINC API = the user's own CINC screen, line for line): last $0 Jul 27, Aug 1 + Sep 1 special-assessment installments unpaid → $674.48 on Sep 8 (38 days), eCheck to $0 on Sep 9. The rule in `lib/application-delinquency-notice.ts` is balance aging ("continuously above zero > 30 days"), NOT the CINC collections/Block-Payments flags. **User decision via AskUserQuestion: keep 30 days as is.** Then built #860: the OWNER notice states the amount and unpaid-since date (new `openBalanceAging()`), the applicant/agent notice unchanged. Not built: an all-clear email when a flagged balance clears while the application is open. Memory: [[application_delinquency_30day_rule]].

### Missing-documents reminder: approval gate removed (PR, 2026-09-11)
User received "Approve reminder — … Unit 903 still missing documents" and asked why, since there was nothing to review. It was the ONE-TIME approval gate on the 3-day "what's still missing" reminder (their own Aug 20 rule 2). User: "remove the gate, send the first reminder automatically." `app/api/cron/missing-docs-reminders`: no more drafting/asking — first cycle sends and logs an auto-approved row; a `pending` draft left by the old gate (903's, 2026-09-11 8:00 AM ET) is sent on the next run and closed as `auto (gate removed 2026-09-11)`; approved/declined rows just set the 3-day clock. Visibility instead of the gate: the 7 AM digest gained "Reminders MAIA sent in the last 24 hours" (awareness only — does not by itself make the digest send). `draftReminderApproval` / the token approval page still exist for any old email link but nothing new calls them. Also flagged to the user: unit 903 has two applicants both named "Shoodlyne Deus" (same phone, different emails — likely a mis-named second person or a duplicate) → doubles per-applicant items and would order two Checkr screenings; staff to correct.
...
- The 4 CINC sync leads flagged earlier this session (`apueyoruiz56@gmail.com`, `victorzje@yahoo.com`, `aimtransportnj@gmail.com` — VPC5 1J vs VPCI 27M/VPREC #27, `mfelipe@marcellfelipe.com`) are **already resolved** — user confirmation, 2026-09-09, correcting this doc's own earlier "still open" note. No further action needed.
...
### Two more real bugs closed out before ending the session (PRs #849, #850)

- **#849 — "second applicant can't upload their own document," finally root-caused.** A `per_applicant` checklist item (`car_registration`, `vehicle_insurance`, etc.) legitimately has one `application_documents` row per stakeholder sharing the same `doc_key`. Three places never respected that: `recordIntakeDoc()` deleted *any* existing row for a `doc_key` regardless of whose it was (so a co-applicant's upload silently deleted a *different* co-applicant's already-uploaded one); the intake checklist's "already done?" check was a flat, application-wide set (so once *any* co-applicant uploaded a per-applicant item, every *other* co-applicant's own checklist showed it as already satisfied and never offered them an upload control — the literal "can't upload" symptom); and the submit-completeness gate had the same flat-check bug. All three fixed to be per-stakeholder-aware for `per_applicant` items.
- **#850 — the `.maybeSingle()` co-owner audit, done in full.** Went through every `owners` query flagged in the prior entry's "worth a wider audit" note. New discovery mid-audit: `account_number` doesn't protect against the bug either — it identifies the *account/unit*, not the individual owner, so it's exactly as vulnerable as `unit_number`. New shared `lib/owner-lookup.ts` (`findOwnerRows`/`mergeOwnerRows`/`findMergedOwner`); fixed ~20 call sites across owner self-service (compliance portal, ACH, ledgers), tenant compliance, pre-apply unit lookup, and five cron/notification flows. Three of these were live, user-facing hard failures for any co-owned unit (the owner's entire compliance portal, the tenant compliance page, and the pre-apply "is this unit on file?" check).

### ⏳ NEXT

1. No live Supabase/CINC/Checkr credentials in this session at any point — every fix here was either verified by the user running staff-only debug routes and pasting results back (the CINC status field, the Checkr `/orders`/`/properties` investigation), or reasoned from code alone (the `is_primary`/`.maybeSingle()`/per-applicant-upload bugs) and confirmed working only via the user's own live testing afterward, or not yet independently re-verified live (PRs #849, #850 — reasoned + typechecked, not yet confirmed against real co-owned/multi-applicant data by the user).
2. **New goal, not started:** an association onboarding questionnaire — timestamped board decisions on every compliance document/MAIA feature (enable or not), governing docs uploaded for initial rule guidance, proactive feature suggestions, and — at the end — a MAIA-drafted revised Rules & Regulations document for the board to formally review and approve. Likely extends the existing `/admin` Association Setup page (`association_intake_documents`/`association_application_rules`) rather than replacing it — review that page's data model first.

Memory: [[cinc_unit_number_not_unique]], [[cinc_non_billable_filter]], [[checkr_payment_safeguard]], [[checkr_property_normalized_name_collision]], [[application_stakeholders_is_primary_per_role]], [[owners_table_maybeSingle_coowner_bug]], [[per_applicant_document_upload_scoping]].
...
Long session, three loosely-connected threads. This entry only documents what this session actually did (PRs #800 onward), from this conversation's own live record. **The PRs #740–#799 gap below this doc's previous top entry (2026-08-31) has since been backfilled** — see the "(backfilled from git history, PRs #740–#799)" entry immediately below this one, reconstructed from `git log` per user request, not a live narrative.

### Checkr report: auto-filed, then made colorful, then merged into ONE pdf (PRs #800–#808, #812–#813, #816)

User's original 4-point ask: file the completed report onto the applicant's own checklist row, and make the preview "more colourful for my dashboard and for the board to review." Shipped in stages:
1. **Auto-file** (#800) — a completed report previously sat only in `screening_subjects`, never in `application_documents`, so the required "Background / Credit Reports" checklist item read as permanently missing.
2. **Colorful summary** (#801, #807) — `lib/screening/report-summary.ts`'s `summarizeReport()` parses Checkr's raw `report_data` into credit score, file/report status, and real `credit_summary` stats (monthly payment, account age, utilization, on-time-payment trend), verified against Querline Pinckney's (MANXI 912) real captured report. Extended with the richer stats only after the user pasted real JSON — the parser was previously guessing.
3. **Never silently hide it** (#806) — the summary component used to return `null` on unparseable data, giving zero indication anything was wrong; fixed to always show a status chip + raw-data toggle, which is what surfaced the real JSON shape in the first place.
4. **Bug: wrong stakeholder** (#802, #804) — `background_credit` is a per-applicant checklist item; brittle name-matching between Checkr's `subject.name` and `application_stakeholders.name` silently left multi-applicant reports unscoped. Fixed with a single-applicant fast-path + normalized matching, then properly closed with a new `screening_subjects.stakeholder_id` column set at order-creation time (exact, no guessing) — NOT sent to Checkr's own API as a `custom_id`/equivalent, since that field isn't confirmed in their docs.
5. **Bug: missing from the checklist row** (#803) — the colorful summary only showed on the top summary card, not the per-applicant row it actually needed to sit next to.
6. **First shipped the WRONG shape (#808), then reworked (#816).** Built as a separate downloadable "🖨 Colorful summary (PDF)" button after an `AskUserQuestion` round whose answer at the time was "Add as a separate Summary PDF." User later tested it and corrected: *"my idea was having the new colourful in the same PDF and Preview as the original, so both could be seen by the board while approving, not a new button that I will never use."* Fully reworked: `lib/screening-summary-pdf.tsx`'s new `mergeWithOriginalPdf()` (pdf-lib) splices the colorful cover in FRONT of Checkr's original report, byte-for-byte unmodified — same compliance stance as `lib/rules-ack-pdf.ts`'s `mergeRulesIntoWrapper`. Deleted both separate-PDF API routes and both buttons; the existing "View report" link and "File as document" button (now backed by `regenerateMergedReport()`) are the only UI. **A report filed before #816 needs "File as document" clicked once to pick up the merged version** — nothing retroactively upgrades on its own.
7. **Open applications stuck on a dead Rentvine panel** (#812, #813) — root cause: `listing_applications.screening_provider` is a frozen per-application snapshot from before the association-wide flip to `maia_checkr`, and `resolveScreeningProvider(raw)` treats `NULL` identically to the literal string `'tenant_evaluation'`. The FIRST backfill migration's `WHERE screening_provider = 'tenant_evaluation'` silently matched zero NULL rows and did nothing — user reported "ran bash - Success. No rows returned" then "still all of them show the Rentvine message," which is what caught it. Corrected with `OR screening_provider IS NULL`.
8. **Manual re-screening resend** (#805) — a staff button to resend the $150 re-screening payment link + countdown, per "don't forget to send the countdown days the same email to them to review."

### Board review correctness + a real process gate (PRs #809–#811, #815)

- **#809 — Decider/Voter (Committee) config, real incident.** The automatic decision-letter signer selection (`lib/board-decision-letter.ts`) picked signers by board-title priority only, ignoring the association's own "Committee — Application Approval" config a DIFFERENT older flow already respected. Confirmed via a real Resend screenshot: Karen Fisher (configured Voter) got a real "Please sign" email; Walter Leslie Joseph IV Giles (configured Decider) did not. Fixed for both the automatic and manual staff decision-page paths.
- **#810 — the letter now requires a real board review round first.** User: "before sending the approval letter Maia should have sent the SEND TO THE BOARD REVIEW - not the letter itself." Staff approving every document directly, with nobody ever clicking "🏛 Send to the board to review," could previously still trigger the automatic letter. New `ensureBoardReviewRoundSent()` sends a round instead of the letter when none exists yet.
- **#811 — the round email shows the real due date once the window is already open**, instead of the generic "may decide up to N days" sentence that would otherwise hide an already-ticking clock (matters specifically when #810's auto-send fires for an application whose window opened before the round ever went out).
- **#815 — staff dashboard visibility.** User asked twice, a session apart: "I still can't see in my dashboard when someone approved all files" then, after that shipped, "we have the name and timestamp of who approved, don't we have?" Both now surfaced on the staff applications list: `windowOpenedAt`, plus WHO finalized it (name + role: Staff/Board/On-site manager) via a new `application_document_reviews` lookup.
- Confirmed separately, no code change needed: the on-site manager already receives the initial per-document review round email (can review + approve, structurally can never be a decision-LETTER signer, which only ever sources from `association_board_members`).

### Owner balance + governing-document rules on the board card (PRs #817–#820)

- **#817 — owner balance + ledger link.** "board wants to see in the card they receive with the files for them to verify and approve... the unit owner balance and a link with his ledger, since they won't approve if the unit is not current." Banner (green/current, red/balance-due) on the board review round email, fetched live from CINC on every send — no separate daily refresh job, since the email is rebuilt fresh each time it goes out. Ledger link is a login-free `signLedgerToken` pointing at the existing owner-statement PDF endpoint.
- **#818, #819 — per-association 30-day/business-day rule text, quoted verbatim.** User asked what the by-laws say happens if the board misses the window; this session could NOT get a verified answer from Florida Statutes/DBPR (WebFetch was blocked to every domain tried in this environment — flsenate.gov, leg.state.fl.us, DBPR, several law-firm sources — and WebSearch summaries disagreed with each other, one wrongly claiming 60 days). Rather than guess at law, asked the user directly; they supplied MANXI's actual "Amendment to Rules and Regulations" PDF (30 calendar days, no stated consequence). VPCI's rule was ALREADY sitting in the repo (`lib/vpci-rules-ack.ts`, sourced from the Declaration Article XXII: 10 business days, deemed approved if missed) — found by checking what was already extracted before asking for more documents. New `lib/board-decision-rules.ts` holds both, verbatim, per-association; nothing is shown for any other association.
- **#820 — VPCI's window actually fixed to run in business days.** Finding VPCI's rule while adding #819 surfaced a real bug: every application, VPCI included, was silently defaulting to the 30-CALENDAR-day window (`board_window_days`), so VPCI's stricter 10-BUSINESS-day Declaration was never actually honored by the real reminder/decision-letter timing. `boardWindowOverrideFor` + `addBusinessDays` (Mon–Fri; no holiday calendar exists in this codebase). `dueAt` is always derived fresh, never stored, so this fixed every VPCI application — past and future — with nothing to backfill. Verified independently via `node -e` before writing the regression-test assertion (2026-08-01 is a Saturday; +10 business days lands on 2026-08-14, not the naive +10-calendar-days answer).

### ⏳ NEXT
1. **The other 23 associations have no confirmed board-decision rule or window-unit override.** Every one of them still silently assumes the 30-calendar-day default, unverified against its actual Declaration/Rules. Get each association's governing-document language on this (paste the relevant page, same as MANXI) to close this out properly.
2. **MANXI's source PDF was never filed** to `associations.rules_pdf_path` / the RULES_BUCKET — only the extracted text lives in code. This session had no live Supabase storage credentials to do that upload; needs a session with production access.
3. **This session had no live Supabase/CINC credentials at all** (no `.env.local`, nothing in the shell environment) — every fix in this entry was written and shipped blind to production data, relying entirely on user-supplied evidence (screenshots, pasted JSON, pasted PDFs) rather than direct verification. Worth confirming with a live-access session that the owner-balance banner and ledger links actually render correctly against real CINC data.
4. Older, unresolved, not touched this session: Checkr key prefix (test vs live) — last verified 2026-09-06, watch for recurrence on any future key rotation; Rentvine tenant-sync cron dead since 2026-06-17.
5. The "second applicant can't upload their own document" bug (reported earlier, out of scope for the vehicle/tax-returns per-applicant work referenced in the gap note above) — still unfixed, unscoped.

Memory: [[checkr_report_merged_pdf]], [[board_decision_letter_committee_fix]], [[board_review_round_required_gate]], [[vpci_business_day_window]], [[board_decision_rules_manxi_vpci]].

---

# Session handoff — 2026-09-01 to 2026-09-06 (backfilled from git history, PRs #740–#799)

**This entry is reconstructed, not a live session narrative.** No handoff entry was ever written for this stretch — user request 2026-09-08: "Backfill PRs #739-799 too - get it from git history." Built entirely from `git log`'s full commit messages (`e7668ca..b5cab42`, squash-merge bodies), which turned out to already carry rich, detailed narrative for nearly every PR — but any color that only ever existed in the live conversation and never made it into a commit message is genuinely lost. `docs/ROADMAP.md` fared much better across this same stretch — many of these PRs included their own "docs: ..." companion commit keeping it current in near-real-time — so it needed only a few corrections (a stale "not yet implemented" header on the Checkr-first pipeline section, one stale "MANXI has not been flipped to Checkr" sentence) rather than a full rebuild.

## 2026-09-01 — declaration backfills, esign fixes, Checkr's own bug confirmed (PRs #740–#747)

Real gaps found while working two live cases, MANXI 303 (Wilner Florestan) and MANXI purchase applications generally: `draftStandardReply()` was still asking for the Landlord-Tenant Agreement as an upload when it's actually an e-signed packet (#740); pre-existing MANXI purchase applications never got the international-tax-returns question (only asked at intake, which postdates them) so their tax-return checklist items stayed permanently "missing" even with the actual document on file — backfill migration + staff-facing edit UI added (#741, #742). A special-assessment charge was winning the "current quarterly assessment" ledger lookup over the real recurring charge on the Maintenance Assessment Acknowledgment, mis-stating the dollar figure a resident signed; fixed, plus a live preview button so staff can check the figure before sending (#743). `application_document_reviews` decisions are keyed to the checklist item, not any one uploaded file, so a refiled document (re-upload, Drive pick, resent e-sign) kept showing the OLD decision forever — `deriveReviewState()` now only honors a decision made on or after the current document's own `created_at` (#744), and the checklist row itself now shows "· filed \<date\>" so staff can see at a glance whether something newer landed (#745). Lease-extract's prompt got an explicit pointer to the standard Florida lease form's contact-info block after tenant email/phone kept coming back null despite being on the page (#746). And Checkr support confirmed the null `credit_report`/`eviction_history` finding from 2026-08-31 was their OWN bug (test-mode data not populating), not an account limitation — flagged as not-yet-reverified pending a live re-check, plus a new "Background check (Checkr)" viewer on the staff page and a key-mode (test/live) diagnostic on `/admin/tools` (#747).

## 2026-09-01/02 — the Checkr-first pipeline redesign, Phases 1–4/6/7 (PRs #748–#754)

Full spec decided with the user 2026-09-01 (see `docs/ROADMAP.md`'s own section, now corrected to reflect what's actually shipped): screening payment moves to the FRONT of the flow instead of firing only after board approval. Phase 1 (#748) gates `/pre-apply/[code]` document upload behind the existing `/apply?listingApp=` Stripe+Checkr handoff, widened same-PR to cover co-applicants and `additional_occupant` (not just the lead applicant) and to require the marriage certificate actually be uploaded — not just the "are you married" toggle — before honoring the $150 couple rate, both client- and server-verified. Phase 6 (#749) prices and screens adult occupants for real ($150/adult, real Checkr subjects) instead of just listing them with a false "you'll be invited to screen" promise — deliberately narrower than the original roster-unification design (documented as real scope-narrowing, not an oversight). Phase 7 (#750) lets a legal guardian/POA holder sign and consent on behalf of an occupant who can't (MANXI 706, Timothy Dean Walker) — rejected both "the applicant just signs for him" and "reuse his email," since this app fingerprints who signs (photo/geolocation/drawn signature) specifically to prove who was present, and either shortcut would fabricate that record; built instead as an explicit "who's actually signing, what's their authority, required guardianship/POA upload, distinct attestation naming the represented person" flow, with a real blocking validation. Phase 4 (#751) is the 45-day screening-validity window (`lib/screening/validity.ts`, the one source of truth for staff/board/the window logic), a 10/5/1-day expiry-warning cron, and the $150 re-screening payment flow with the required non-"application fee" legal framing. Phase 2 (#753) adds a real e-signed acknowledgment (not just warning text) covering the checklist shown and the 45-day deadline, gating the payment button. Phase 3 (#754) closes a real access gap: the board queue's LIST already hid unsubmitted applications, but the detail/action endpoint behind `/units/applications` had no matching guard, so a reviewer with a bookmarked/guessed id could open — even approve or decline — an application the applicant hadn't finished submitting; deliberately left the existing per-document review-round workflow on already-submitted applications untouched. #752, same window: staff now has to explicitly confirm before approving a document flagged expired (previously one click, same as any other), and every e-signed checklist row shows real signer-status ("Signed by X" with a date, or "waiting on Y").

## 2026-09-03 — a very large single day: unit confirmation, phone/SMS-only applicants, retiring Tenant Evaluation, the Checkr blocker fully resolved (PRs #755–#777)

The single densest day in this range, almost entirely traced from real staff reports on Querline Pinckney's MANXI application and a few others. In rough order:
- **#755**: unit number becomes required + confirmed (was free-text/optional, silently skipping the duplicate-application and owner-verification checks); fixed a hardcoded association name on the welcome hero; per user direction, the guardian sign-on-behalf-of flow drops its required-upload gate down to attestation-only (getting paperwork isn't always realistic in the moment that matters); the whole `/pre-apply` flow becomes completable over SMS/WhatsApp for anyone with no email, reusing the existing Twilio OTP infrastructure; the agent proxy-upload workflow gets clearly labeled ("upload on {client}'s behalf") instead of reading as an edge-case convenience; and — after all that — email becomes REQUIRED again specifically for anyone who'll be background-checked, since Checkr's own consent/questionnaire delivery is email-only.
- **#756–#758**: dropped the redundant Gov ID/Proof of Income upload from `/apply` (the association's own post-payment checklist already collects it, configurably); added 20s timeouts to every CINC API call (a slow/degraded CINC was hanging invoice-push requests for the caller's full function budget); board review's Government ID field now reads from the real post-payment checklist instead of a permanently-empty legacy field, and Proof of Income was dropped from board review entirely since no association's checklist actually collects it; fixed a real bug where the filed Landlord-Tenant Agreement PDF could show the completing signature as still "Awaiting" (rendered from a stale in-memory copy fetched before the save) — added a staff "Re-file PDF" recovery action.
- **#759**: split the expired-document banner so the Landlord-Tenant Agreement (whose "expiration" is the lease's own end date, not a stale signature) stops reading as something to re-request; added a staff "ask her to answer" nudge for an unanswered declaration; added a free-text search bar to the applications queue; added a staff button to manually fire a Checkr order (MANXI 912 had no automatic path working yet); made the Landlord-Tenant Agreement self-service like the other e-signed items (was falling through to a plain, unfillable upload box).
- **#760**: stopped an N/A'd checklist item (e.g. Pet Registration retired in favor of the ESA/accommodation path) from still tripping the EXPIRED badge with no way to clear it; retired Tenant Evaluation from the UI (code kept, just not rendered) in favor of Checkr, and added a generic "Rentvine fallback" manual-link option as the alternative.
- **#761**: **the Checkr credit/eviction blocker is resolved** — a live sandbox order came back with both populated, Checkr confirmed in writing they're on the package, and MANXI is flipped to `screening_provider: maia_checkr`. Same PR: `listing_applications.screening_provider` becomes a real per-application SNAPSHOT taken at creation (`resolveScreeningProvider()`), specifically so that flipping an association's default going forward never silently re-routes an application already in flight.
- **#762–#764**: updated the MANXI application guide for the new Checkr-first flow; surfaced the raw Rentvine fallback URL for staff's own use, not just an emailed link; corrected two stale ROADMAP claims and relabeled the retired screening toggle option; moved the association's document-checklist config into an owner-only tab (`requireOwnerSession`, a real allowlist check on the API routes too, not just a hidden tab); persisted "ask her to answer" reminders into the real Communication history instead of forgetting them on reload; added a one-off per-application Checkr switch for an application still snapshotted onto the retired provider.
- **#765–#769**: staff now get emailed when a Checkr check completes/fails/goes partial (nothing did this before — only the database updated); Communication history collapses by default and now logs Checkr requests; applicants get one MAIA-authored heads-up before Checkr's own separate consent email arrives; "payment not done yet" turned from a dead-end error into a real "resend the payment link" action (root cause: her application predated the payment gate entirely, so paying was never actually in her workflow); the `/apply` wizard now prefills from her pre-apply record (name/email/phone, marriage cert, rules-ack) instead of re-asking everything; fixed two real bugs in "send the confirm & pay link" (missing state refresh, a Communication tab that never learned anything new landed without a full reload).
- **#770–#772**: Gmail add-on gets a one-click new-application-link generator; fixed three live add-on bugs found by staff testing it (unit as a real dropdown not free text, a "linked ticket" that was matching an unrelated thread, one unified Create flow instead of a bolted-on second feature) across two follow-up commits, the second one giving Application Link its own dedicated button after Priority/Assignee/Subject looked required when they weren't.
- **#773–#775**: MANXI's Tenant Affidavit checklist item, present for `lease`, was missing entirely for `additional_occupant` — migration copies the row; the owner ACH form now checks CINC's real billing-type before asking someone already enrolled to re-enroll (Andy Sahadeo, MANXI 409, got a confusing "please set up" email despite being on autopay since 2024); the Gmail widget's Application Link draft now also includes the Application Guide when one exists.
- **#776**: removed the misleading "@maia upapp" forward-to-file footer from every resident-facing email AND the public `/request/[token]` page — it was only ever meant for staff (files a forwarded email's body/filenames, never the actual attachment content) and a real applicant (Mark Leguizamon, MANXI 613) lost a document that way, believing it was filed. Also redacts SSN-shaped text from the public "leave us a message" box after an owner used it to relay a tenant's SSN in plaintext.
- **#777**: staff now get alerted when an outbound MAIA email actually fails to send or later bounces/complains (both failure modes previously either silently swallowed by callers' own `.catch()` or recorded with no notification) — throttled to one alert per 10 minutes so an outage can't flood staff with one email per failure.

## 2026-09-03/04 — reliability + wizard polish (PRs #778–#780)

Email addresses now get ALL whitespace stripped, not just leading/trailing (a stray internal space silently made Resend reject a whole send — only caught after the fact by #777's new alert); fixed a document preview that clipped a raw phone-screenshot upload instead of scaling it down (#778). `/apply` gains: a non-refundable disclosure, the ability to add documents after paying (payment was never actually the final step), fixed a stuck-on-"Loading..." state, distinguished a genuine backend failure from a not-found in the logs, and a real per-association link `/apply/[code]` (#779). The wizard itself gains a manual unit-number field, reordered application-type cards, and — the significant one — a new pre-payment Summary step recapping type/applicant count/married status/price with edit-back links, so an applicant sees and can correct the charge before reaching consent, not just at the moment of paying; plus a persistent Back button, corrected success-page copy (the old text described a "board reviews and votes" process that isn't how this actually works), and a required Application Guide acknowledgment for MANXI applicants specifically (#780).

## 2026-09-04/05 — vehicle and tax-returns answered per applicant, not once for the whole application (PRs #781–#782)

Real bug: `car_registration`/`vehicle_insurance` and the international-branch tax-return documents are already `per_applicant` checklist items, but the underlying YES/NO answer was one shared field for the whole application — so one co-applicant's "yes, I have a car" opened a waiting row for every co-applicant, including ones who'd have answered "no." New `vehicle_has`/`tax_returns_has` (+ `_declared_at`) columns on `application_stakeholders`, with the existing shared `declarations` value read as a fallback for the primary applicant only (so already-answered legacy applications keep working). Covers the intake flow (#781, real per-stakeholder identity already exists there via `signPreApplyToken`) and, in a same-day follow-up, the staff-triggered `/request/[token]` follow-up link too (#782) — that link only ever had role-level identity (owner/tenant), so it resolves the declaring stakeholder by matching the sender's email against the application roster, falling back to the old shared-field behavior when it can't. Animal stays a single, application-level answer throughout — deliberately not touched, since it isn't a per-person fact the way a vehicle or a tax return is.

## 2026-09-05/06 — dashboard visibility, a second Checkr key-prefix saga, defaulting every association to Checkr (PRs #783–#799)

- **#783**: a failed Supabase query (e.g. selecting a column a migration hadn't been applied to yet) read back identical to "no rows" — exactly how a co-applicant's entire roster silently vanished from the dashboard once the vehicle/tax-returns migration lagged the code. Every `application_stakeholders` read in the hot paths now logs its error instead of swallowing it. Same PR: the Applications list now shows every applicant on a multi-applicant application, not just whoever happened to be primary.
- **#784**: the plain-text Stripe payment receipt (Querline Pinckney's ONLY email at that point) still described a stale "board reviews within 7-10 days" process with zero mention of the background check or documents step that actually comes first — rewritten to match the real four-step flow.
- **#785**: the assistance-animal Fair Housing guidance block (what may/must never be asked, narrow refusal grounds) collapsed behind a disclosure toggle, off by default — kept in full, just not always inline, since it's the actual guardrail against a board member asking for something the law forbids.
- **#786–#789, then #798 (the reversal)**: a second real Checkr key-prefix saga. A live applicant's order silently ran in sandbox mode because `CHECKR_API_KEY` was still the Jul 6 test key (#786's root cause) — while fixing it, the key-status diagnostic kept reporting "unrecognized" even after a clean paste, wrongly "fixed" from `ckr_sk_test_`/`ckr_sk_live_` to `sk_test_`/`sk_live_` based on a TRUNCATED display in Checkr's own dashboard table (not the real secret) — #787 added length/char-code diagnostics chasing the same mystery, #788 surfaced the (non-secret) package-tier value, #789 let staff re-request an order past a stale test-mode one. **#798, same day**: caught and reverted the wrong prefix fix — a raw copy-to-clipboard of an actual live key confirmed the real prefix genuinely does include `ckr_` (52 chars total) — after the wrong check had staff strip it, which surfaced downstream as a real `401` from Checkr's own API on a live attempt. `CHECKR_API_KEY` needed re-pasting in Vercel with the full prefix restored.
- **#790**: Checkr's full status-event history (not just the latest) and payment status surfaced in the admin UI — previously required pulling Vercel logs to see what had actually been received over time.
- **#791**: every MAIA email now defaults a real reply-to (`support@`) instead of silently falling back to the inbound-command mailbox when a sender replies; Jonathan added to the board CC list everywhere PMI already was; removed the now-pointless "Scan Drive folder" feature (nothing is dropped into a Drive folder by hand anymore — every application originates inside MAIA's own intake flow).
- **#792**: the green "approved" dot now also shows on a multi-file item's extra files (was primary-file-only) and on an N/A'd item that was already fully e-signed beforehand (previously looked untouched either way).
- **#793–#794**: a real "I am totally blind to this" user report drove two additions — staff's own applications list finally renders the alarm/days-left/letter-progress data `getApplicationDashboard()` was already computing but never showing (board/on-site-manager's own portal already had it), plus a new, distinct **interview** stage so a stalled required-interview no longer looks identical to a plain "write the letter" wait; and the daily digest gains two new sections (30-day window already passed with no letter yet; stuck 14+ days on a required interview) — no new cron, reuses the existing 7am ET send.
- **#795**: the Checkr request button and its confirm dialog now say whether the applicant already paid and make explicit that a paid application is never charged again — staff had asked "will it charge her again?" with no way to tell from the UI.
- **#796**: **all associations default to `maia_checkr`**, a real double-trigger Stripe-webhook bug fixed (it fired Checkr unconditionally on payment regardless of an application's actual resolved provider), and Tenant Evaluation fully deleted from the codebase (not just hidden) — see `docs/ROADMAP.md`'s Checkr section for the durable-state summary of all three.
- **#797**: the migrations list on `/admin/tools` collapses already-applied ones by default (staff report: too long to scan), and fixes a real bug this surfaced — Show SQL/Apply only rendered for a migration reading `!applied`, so a DATA-only migration (an `UPDATE`, or a column-default change with no new column to gate on) that already happened to read "applied" couldn't be re-shown or re-run at all.
- **#799**: the "Request background check via Checkr" button now fades (still clickable) once every applicant already has an approved background-check document on file through some other channel — was fully active with no signal it likely wasn't needed, a real risk of double-ordering a separately-charged report.

Memory: [[checkr_first_pipeline_redesign]], [[tenant_evaluation_retired]], [[checkr_key_prefix_saga]], [[vehicle_tax_returns_per_applicant]], [[applications_dashboard_alarm_visibility]].

---

# Session handoff — 2026-08-31

## FabiOS architecture review, HO-6/DP-3 insurance acceptance, car-registration spot-check, and the Checkr credit-report investigation (PRs #735–#738)

Four independent threads in one session, each landed as its own PR against `main`.

### FabiOS architecture review (`docs/FABIOS-ARCHITECTURE-REVIEW.md`, PR #735)
User is designing FabiOS, a separate AI marketing/authority-building system for Fabio Setton/PMI, and asked for a full technical inspection of MAIA to figure out what a safe MAIA → FabiOS knowledge boundary should look like. Dispatched parallel research agents across Gmail, Google Drive, Follow Up Boss, work-order/media, and infrastructure, then wrote up all 32 requested sections (A–AF): architecture summary, privacy classification (5-class taxonomy), the security boundary design (separate Supabase project + one-way approval-gated event API + isolated media folder), database model, phased build plan, and — per the user's own section AF — exactly what's safe to copy to ChatGPT for a second review versus what must stay out. Key findings, all verified by reading the actual code rather than assumed: **Follow Up Boss is not integrated anywhere** (zero references in code/docs/migrations — this had been treated as possibly-already-connected before checking); Gmail/Drive are live but purely reactive, no historical crawl or embeddings exist; infrastructure is Vercel-only, no Docker/Redis/vector-DB/n8n/OpenClaw despite earlier assumptions. Sent both the full report and a sanitized reference-file package (CLAUDE.md, package.json, vercel.json, .env.example structure, folder tree) for the user to hand to ChatGPT — real association/unit names appear throughout the actual codebase, so full source files were deliberately excluded from that package.

### HO-6 / DP-3 unit-owner insurance acceptance (PR #735)
User forwarded a real Citizens "Dwelling Fire DP-3 Unit Owners Special Form" policy (MANXI unit 912, Carmen Robinson) and asked whether DP-3 should be accepted alongside HO-6 — correctly caught that MAIA's insurance-validation copy/prompts only ever named "HO-6" by itself. Fixed everywhere it mattered, most importantly `lib/document-validation.ts`'s AI upload gate (the actual path an owner's insurance upload goes through, `app/api/units/documents/submit`) — previously told the model to expect "HO-6" only, the one real path that could have auto-rejected a legitimate DP-3. Full list in `docs/ROADMAP.md`'s new entry. Separately, user pointed out the specific checklist item Carmen Robinson's document was already filed under ("HO6 Property Insurance," MANXI/lease) needed the same fix in its label text — `20260831_property_insurance_ho6_or_dp3_label.sql`, applied by hand and verified via a follow-up `SELECT`: exactly the 1 targeted row changed, all 17+ other associations' rows correctly untouched (scoped to the exact old label text, matching this repo's established migration-safety pattern).

### Car-registration spot-check, closed out (PR #735)
Picked up the open ROADMAP item from the 2026-08-22 Haiku→Sonnet fix: documents scanned before the fix might carry wrong/blank expirations, with no error-vs-empty distinction stored to find them by query. Built `scripts/spot-check-car-registrations.ts` (dry-run by default, idempotent, follows the existing `backfill-cinc-wo-details.ts` pattern) — couldn't execute it directly (no Supabase/Anthropic credentials in this environment), so did the actual check by hand: SQL query narrowed the portfolio-wide scope to exactly 2 documents, then the user uploaded both stored images directly for a real visual read. MANXI 613 (Mark Anthony Leguizamon) was already correct (`2027-03-08`, matching "Expires Midnight Mon 3/8/2027" — this was the original incident's already-corrected value). MANXI 912 (Querline Yasmine Pinckney) was wrong — stored `2027-11-03`, actual printed date `2028-03-14` — corrected live via a scoped `UPDATE` (matched on both `id` and the known-wrong old value).

### The Checkr credit-report/eviction-history investigation — four rounds to a conclusive answer (PRs #735–#738)
Continuation of the 2026-08-30 finding (`credit_report`/`eviction_history` came back `null` on a test order). Victor from Checkr support replied pointing at the documented Canned Provider Scenarios testing guide — kicked off a real investigation with several genuine course corrections along the way, each one driven by live evidence rather than assumption:

1. **Real bug found and fixed (PR #735).** `checkr-tenant-api-docs.redocly.app/testing#canned-provider-scenarios` (fetched via the user pasting screenshots, since the domain is egress-blocked from this environment): a canned scenario only fires on an EXACT match of first_name+last_name+dob+ssn together — a partial match silently falls through to an inert "clean scenario for every product." `app/api/admin/applications/create-test/route.ts` had exactly this bug in two places (the default test applicant and the custom-name override both reused a documented canned SSN paired with the wrong name/DOB). Fixed; added `getReport()` (MAIA had never had a code path reading the structured report body at all — only the rendered PDF) and two new test scenarios (Madelyn Webster, Ingrid Vance).
2. **First conclusion was premature.** Assumed the tuple-mismatch bug fully explained the original null finding and said so in the PR. User ran the fix live and it wasn't that simple.
3. **Live re-test with the exact tuple still showed nulls (PR #736).** Madelyn Webster — the documented "credit: consider" tuple, matched exactly — still returned `credit_report`/`eviction_history` as `null`, on a report Checkr itself marked complete (criminal history and global watchlist populated correctly on the same order, proving the matching fix genuinely worked). Pulled the full Orders API reference from the user's screenshots (Orders, Reports, Products schemas including `credit-report`, `eviction-history`, `income-verification`) and found `add_on_products` is a *validated* field — "Unknown or non-add-on products return a validation error." Added optional `ScreeningSubject.addOnProducts` plumbing and had `credit_consider` request `add_on_products: ["credit_report","eviction_history"]` as a live, decisive test — deliberately NOT wired into real production orders (add-ons are typically billed per-use, needs explicit cost sign-off first).
4. **The order got stuck at "pending" with the real error swallowed (PR #737).** User ran it and reported an application row created but nothing else — order creation was failing but only `console.error`-ing server-side, invisible without Vercel log access or DevTools. Spent real effort walking the user through Chrome DevTools Network tab (undocking, filtering) before switching strategy to the actually-right fix: `trigger-screening` now returns the real per-subject error text, and `/admin/applications`'s Test Environment panel shows it directly on the page in red instead of the generic "Created... Reloading."
5. **Conclusive answer (PR #738).** Re-ran once more — got a real `422` straight from Checkr: `{"errors":[{"code":"validation_error","detail":"Unknown add-on products: credit_report, eviction_history","source":{"pointer":"/add_on_products"}}]}`. **This settles it**: these two products are neither bundled in this account's "Essential" package nor purchasable as add-ons — contradicting Checkr's own pricing description and this repo's `.env.example` comment (now corrected). Reverted `credit_consider`'s now-known-futile `add_on_products` request so the scenario is useful again for its original purpose; kept the `addOnProducts` plumbing itself since it's a real, working capability for any future add-on hypothesis. Corrected `lib/screening/checkr.ts`'s header, `.env.example`, and `docs/ROADMAP.md`'s Checkr entry (which had prematurely said this was "resolved" after round 1 — corrected to reflect the real final answer).
...
2. **Checkr:** live key generated and set 2026-09-06 (real case: a MANXI applicant's order silently ran in sandbox mode, no real email or background check ever went out — found via `/api/checkr-webhook` never receiving traffic). Mid-diagnosis, `/api/admin/checkr-key-status`'s prefix check was WRONGLY "fixed" from `ckr_sk_test_`/`ckr_sk_live_` to `sk_test_`/`sk_live_`, based on Checkr's dashboard table showing a truncated `sk_live_••••xxxx` for a key row — that's a shortened display, not the real secret. A raw copy-to-clipboard of an actual live key confirmed the true prefix genuinely includes `ckr_` (52 chars total). Reverted back to the original, correct check the same day, after the wrong one caused a real `401 Invalid token` from Checkr's own API on a live re-request (staff had stripped the real `ckr_` prefix to satisfy the broken check). Re-check that status endpoint, with `CHECKR_API_KEY` re-pasted with its full `ckr_` prefix, to confirm it reports `live`.
