...
## ⚠️ Checkr — integration re-proven working, TWO blockers before going live (updated 2026-09-01)

**UPDATE 2026-09-01 — Blocker 2 below is now IN QUESTION, not resolved.** Checkr support (William, Checkr Tenant) replied: "Our team found that a recent change made it so test mode credit reports and eviction checks were not populating correctly. We just pushed a fix and were able to verify in production that these are once again showing up." That directly contradicts the 2026-08-31 conclusion below (a genuine account/package limitation) — it says the null-report finding was a bug on CHECKR'S side, in test mode specifically, now fixed. **Not yet re-verified on our end.** Next step: re-run create-test with the Madelyn Webster tuple (or similar) and check `screening_subjects.report_data` for non-null `credit_report`/`eviction_history` before treating this as actually resolved. The separate `add_on_products` 422 finding (Checkr rejects `credit_report`/`eviction_history` by name as add-ons) is a different mechanism and may still hold even if the base-package null-report bug is fixed — don't assume one implies the other.
...
**Blocker 2 (2026-08-30) — CONFIRMED 2026-08-31, superseded-but-unconfirmed 2026-09-01 (see UPDATE above): this Checkr account genuinely does not have credit_report or eviction_history, on the package or as an add-on.** Real progress happened in two rounds before landing here:
...
**Do not set any association's `screening_provider` to `maia_checkr` until BOTH blockers are resolved.** A sandbox key returns canned results for real people — worse than an error, because nothing looks wrong; missing credit/eviction data would leave the board deciding on an incomplete picture. Offered, not built: an admin diagnostic reporting only the key prefix.
