...
_Last updated: **2026-07-02**. Status key: ✅ Live · 🟡 Partial · 🔴 Not built · ⚠️ Blocked · ⛔ Decided off._
...
- **Voice Flow diagram** (#486, admin: Tools → Voice Flow) — clickable SVG reference diagram of the IVR call flow; clicking a node shows the real spoken sentence (or notes it's LLM-generated with no fixed script). ⚠️ **Needs a follow-up update** — it still shows the pre-menu-first flow (free-speech classification, Sonnet-routed options 3/4) from before #488/#492 shipped; an update was drafted locally but never committed. Pick this up next session before quoting the diagram as current.
...
- ✅ **Menu-first redesign 2026-07-02 (#488, #492)** — supersedes the "category menu as fallback" design below. Every known caller now goes straight from the greeting to the fixed **1–5 category menu** (maintenance · payments/balance · new tenant/buyer application · association documents · leave a message) — free-speech classification as the primary router was dropped (unreliable even on Sonnet 5), except a quick emergency check that still bypasses the menu. Options 3/4 are fixed scripts (real texted links, no LLM); 1/5 go to the Sonnet 5 answer engine; 2 is the ledger flow. **SMS/WhatsApp got the same menu** (#492) after role resolution, replacing a bare "what do you need?" — free text still routes normally there (no reliability problem on text). ⚠️ The Voice Flow diagram (`/admin` → Tools → Voice Flow) has NOT been updated to reflect this yet.
...
## Suggested priority
1. **Update the Voice Flow diagram** (small, avoids staff quoting a stale flow) → 2. **Second WhatsApp template approval** (`pmi_voice_info_send`) + wire into `sendWhatsAppFromVoice` → 3. **COI validation PR2b** (block + Karen override — small, finishes an already-shipped feature) → 4. **Estimate board report with images** (near-done quick win) → 5. **service@ email-from-WO** (completes vendor procurement) → 6. medium WO/recurring items → 7. Compliance Phase 2 (deadline-rules + document RAG) → 8. smaller comms/invoice follow-ups.

**Verify on next real call:** the menu-first voice redesign (#488) — confirm the category menu feels natural for a known caller (no more open "how can I help"), and that options 3/4's texted links actually arrive. Also confirm the WhatsApp ledger nudge lands reliably now that `pmi_ledger_nudge` is approved + wired (#494) and `TWILIO_LEDGER_NUDGE_TEMPLATE_SID` is set in Vercel.
