...
_Last updated: **2026-09-15**. Status key: ✅ Live · 🟡 Partial · 🔴 Not built · ⚠️ Blocked · ⛔ Decided off._
...
One Applications screen (Open / Approved & closed, Checkr column, search kept; #854, tile count #856) · staff application emails cut to association + unit + link, no per-document mails, daily digest (#855) · Checkr report auto-filed onto EACH applicant card, "File as document" removed (#857, #862) · `a;b` recipient split + "Documents to review" chip (#858) · upload a board-signed approval letter + staff sessions carry a real name (#859) · owner delinquency notice with amount + since-date, 30-day rule kept (#860) · board identity + emails via `lib/board-roster.ts`, legacy `board_members` demoted to phone book; Drive import stops reading Additional-Resident letters as leases (#863, #864) · board card shows unit balance + ledger link (#865) · MANXI purchase letter states advance maintenance by credit score, "paid at closing, title company holds/remits per estoppel" (#866) · missing-docs reminder approval gate removed (#868) · duplicate applicant rows merge on Remove; signature reminders carry per-signer links + "AI Pre-Audited by MAIA" / named board approvals; board card gets a Sign button (#869) · reminder re-checks the active roster and sends a "Waiting for your approval" variant when no letter exists yet, staff "Remind the board now" button (#871) · board card reconciles reviewers with the roster on load + MAIA mark (#872, #873) · days-left counts whole days (#874).

**2026-09-12 additions (PRs #881–#886):** staff "Preview letter" now includes the advance-maintenance block (#881) · applicant-side links accept documents and answers after submission — `intakeClosed()` gates on closed statuses only; reminders list each person's OWN items and, greyed, what waits on the applicant(s) (#882, MANXI 1002) · Manors XI no longer re-screens renewals (checklist row off); a renewal requested > 30 days after the previous lease ended opens as a new lease with screening + fee (`lib/lease-renewal-rule.ts`, #883/#885); the eight renewals open before 2026-09-12 are grandfathered (user decision, #886) · CINC sync flags MAIA-only owner rows and lets staff archive them with a labelled Archive toggle (#888, #891); owner names now come from BOTH CINC address rows with partial-name matching and pooled emails, property-row-only names proposed but not pre-selected (#890, #892) — 2026-09-13. Five archived by user decision (BHB 5692, ESSI 19, LFA 05, ONE 601, WBPA 470K); 5 flagged rows + 3 property-row-only inserts left for staff review on the Hubs; tenant/management CINC rows ignored (#895); non-billable status cached on owners and checked on every row (#896); selections rebuilt after Apply (#894) · **Withdraw** action for applications, silent, nothing deleted (#898) · **Accounting → Application payments** reconciliation (Stripe live, Checkr receipts by order id, Karen's bank ticks) + webhook Checkr fallback without a pre-apply bridge (#901–#905) · **Document scan** (#906–#908, 2026-09-14): registration/insurance expiration = printed Expires line, else Issued/Effective + 1 year; model returns dates with captions, code decides; Sonnet 5 (no `temperature`); 0-byte uploads refused server-side (#907) and caught before upload on both resident pages with iPhone advice (#910). · **Daily News** (#912): counts since the previous edition (prev weekday 5 AM ET), Late = idle past priority window, daily reconciliation tickets superseded (canceled) by the next day's — 140 leftovers canceled 2026-09-14, corrected edition re-sent. · **Lease reminder emails**: one button per answer + Confirm on the check-in page; `unit_occupancy` writers fixed to the account number (check-in + unit survey wrote the label; 10 rows repaired). · **Shared board mailbox** `associations.board_contact_email` (MANXI seeded): interview introduction + invite CC the mailbox, BCC members/managers. · **Processing audit** card + public share link on every application (6 facts, median approval speed, where the days went); 303 carries a Tenant Evaluation timeline. · **Housekeeping** page (MAIA × Drive): stale applications → `expired` (silent, reversible), duplicate/orphan/missing On Going folders fixed with one click; Tenant Evaluation retired. · **Automatic expiry** (cron 6 AM ET): notices then expiry (no files 7d · unpaid 48h · stale 21d+7d · screening 45d+7d), Reopen restores Drive files, digest lists them; legacy unpaid rows hidden after 7d or at once when the same email has a paid form; "Paid elsewhere" records a Tenant Evaluation screening (1003/303/603 done). · **Renewal expiry**: 24-hour violation warning to owner + tenant, then owner / board / manager / staff notified on expiry (2026-09-15). · Applicant done screen lists what waits on the others + "Add or replace a document"; expiry notices per person (1002 owner incident #2). · Document requests prefill and send to every applicant, not just the lead (#933). **Open follow-up:** 103, 1002, 115 are on a 7-day stale notice until 2026-09-21; the notice/expiry days live in `RULE` in `lib/application-auto-expiry.ts` if the board wants different windows. **Open follow-up:** 34 of 37 open work orders and 33 of 36 open tickets are idle past their window (Paola's + unassigned WOs, Fabio's 12 tickets) — the report is honest now; the backlog itself still needs triage.
...
## ✅ LIVE — Checkr-first application pipeline redesign (decided 2026-09-01; Phases 1–4/6/7 shipped 2026-09-02, **Phase 5 shipped 2026-09-15** — all seven phases now built)
...
### Lease non-renewal — ✅ SHIPPED 2026-09-15 (`lib/lease-escalation.ts`)

**The timeline changed at build time.** The user reset it (2026-09-15): *"after the end of the lease, they have 15 days to renew, then more 30 days to have the application expired if they don't present any document and the renewal is not active."* The escalation therefore starts at **T (the lease end date)**, not at T-30 as the diagram below drew it — at T-30 the check-in row is created the same day the first reminder goes out, so literally nobody has had a chance to answer yet and every unit would escalate instantly. What shipped:

- **T** — the owner (only the owner) gets the escalation notice: the unit has no approved lease, a tenant occupying it without one is a violation the owner is responsible for, answer by T+15 or the Board is asked to decide on a violation fee. Same one-click buttons as the existing check-in (`choicesFor('owner','expired')`), so "the lease was already renewed and I didn't tell you" closes it with no fee.
- **T+15** — staff + board (shared mailbox when set) + on-site manager get the decision. Pre-authorized → the fee is stamped and **AR is emailed to post it in CINC**: `lib/integrations/cinc.ts` reads homeowner ledgers and has no charge-write, so MAIA records the authorization and never pretends to post it (user direction, 2026-09-15).
- **T+45** — still no document and no active renewal → the linked application is expired through the existing `expireApplication()` (silent, reversible from Leasing → Housekeeping), the row goes **red** on the staff screen, and anything filed from then on is a new lease — which `lib/lease-renewal-rule.ts` already enforces independently.
- **The clocks stop the instant the owner answers anything at all** — the escalation is about an unresponsive owner, not about the lease.
- **Backlog guard, not in the original spec.** Measured against live data before shipping: a literal implementation would have escalated **24 owners on the first run**, the oldest a lease that ended April 2024. Those are stale `unit_tenant_contacts` rows, not owners ignoring us. The cron only escalates leases that ended within `ESCALATION.backlogDays` (90); older ones and units with no owner email are listed as **backlog** on the staff screen with an "Escalate now" button. First run is 4 automatic, 20 backlog, 17 missing an owner email.
- **Staff screen**: `/admin/lease-escalations` (Leasing → Lease escalations) — the clock, the Authorize fee / No fee / Undo buttons, the red rows, and the backlog list. Cron `/api/cron/lease-escalations` daily at 13:30 UTC, 90 minutes after the renewal reminders so an owner who answers that morning is never escalated the same day. Migration `20260915_lease_renewal_escalation.sql`.

The original design, for reference:

#### Original spec — a NEW, real deadline on top of the existing (currently toothless) reminder cron
...
### Roster-based applicant/occupant pricing — 🟡 shipped 2026-09-02 (narrower than the original design), PR #749
...
### Power of attorney / legal guardian signing — 🟡 shipped 2026-09-02, PR TBD

Real case: MANXI Unit 706, Timothy Dean Walker, an elderly additional resident who cannot sign or consent for himself. The question that surfaced this — whether the applicant can just sign on his behalf, or reuse his email so a family member completes his step under his name — is only partly a UX question. The rest is legal: whether someone else can bind him to a background/credit/eviction-check authorization and a lease Rules & Regulations acknowledgment at all depends on whether they actually hold legal authority to act for him, which is a question for PMI's counsel and the association's governing documents, not something to assume.

**Why reusing the occupant's email is specifically the wrong move, not just an inelegant one**: this app already fingerprints who signs — `rules_applicant_photo`, `geolocation`, and the drawn `rulesSignature` in `components/ApplicationForm.tsx` all exist to prove who was physically present when the Rules & Regulations acknowledgment happened, and `/pre-apply`'s per-stakeholder flow adds an email-OTP step before that person's own SignaturePad unlocks. Letting someone else complete Timothy's step under his email doesn't skip a formality — it fabricates the record: it says Timothy's photo, Timothy's location, Timothy's signature, when it wasn't him. That's worse than no signature on file, because now false evidence exists if it's ever challenged.

**Target design** — an honest "signing on behalf of" path, decided 2026-09-02: the **primary applicant** is the one who identifies this, since Timothy can't do either step himself:

- On each adult occupant row (the roster from the section above), a new question: **"Can this person sign and consent for themselves? Yes/No."**
- If "No": capture **who** is actually signing on their behalf (defaults to the primary applicant's own name, editable — it doesn't have to be them), **their relationship / legal authority** to that person (free text — "Legal Guardian," "Power of Attorney," "Health Care Surrogate," or whatever actually applies; deliberately not a fixed dropdown, since the real range of valid authority isn't enumerable in the UI), and **required document upload** — the actual guardianship or POA paperwork, same upload-gates-the-claim pattern already used for the marriage certificate (PR #748) and Sunbiz registration.
- A distinct, explicit attestation — not folded into the general "I agree to the terms above" consent — naming the represented person: *"I confirm I am legally authorized to sign and consent to a background, credit, and eviction check on behalf of {name}."* Checked separately from the general consent, and blocks continuing to Payment until it's checked AND the document is uploaded (a real blocking validation — unlike most of this form's fields today, which are marked required but not actually enforced in `handleNext`; this one is deliberately stricter given the stakes).
- The photo/geolocation/signature capture at the Rules step is untouched — it now honestly proves who signed and, combined with the guardian attestation + document, who they signed for and under what claimed authority. Nothing here removes evidence; it adds the missing piece instead of hiding behind someone else's identity.
- Checkr screening for that occupant proceeds exactly as normal once payment completes — the guardian's consent stands in for the occupant's own, the same way a parent's consent already stands in for other legally-dependent screening contexts. No change needed to `trigger-screening` itself.

**Where it lives**: the same adult-occupant roster on `/apply` this builds on top of (PR #749) — new fields per occupant (`canSignSelf`, `guardianName`, `guardianRelationship`, `guardianConfirmed`) plus a `guardianDocUrl` upload alongside the existing Gov ID / Proof of Income uploads for that person.

**Deliberately out of scope for this phase**: extending this to co-applicants (`applicants[]`, the `couple` case) or to `/pre-apply`'s per-stakeholder signing flow. The real case is an additional occupant; both of those are natural follow-ups if the same need comes up there, not built speculatively now.

**Open question for PMI, not a product decision**: whether an informal "family member assisting, no POA/guardianship paperwork" case should be allowed at all, or whether the document upload should be a hard requirement with no exception. This phase assumes the latter (required, no informal path) since that's the only version that doesn't ask MAIA to make a legal judgment call it isn't positioned to make — confirm with counsel before loosening it.

**Shipped exactly as designed above** — every bullet in "Target design" built as specified: the Yes/No toggle, free-text signer name + relationship (defaults to nothing, not auto-filled with the primary applicant — a real decision each time, not a rubber stamp), the distinct checked attestation naming the occupant by name, the required `guardianDocUrl` upload, and a real blocking validation in `handleNext` (unlike most of this form's fields) — it won't let the applicant reach Payment for an occupant marked `canSignSelf === 'no'` without both the attestation checked and the document actually uploaded. `trigger-screening` untouched, as designed — the guardian's consent stands in, screening proceeds normally once payment completes.

**Confirmed during this build, not a new decision**: the wizard's step order stays as-is (Applicant Info → Documents+Consent → Payment). A real question came up about whether Payment needed to move earlier for occupant pricing/consent to work — traced end to end and confirmed it doesn't: occupant data (from Applicant Info) and consent (Checkr's own per-subject hosted consent page, independent of PMI's wizard) are already correct under the current order. No reorder needed or made.

### Phasing

1. **Pipeline unification** — 🟡 **partially shipped 2026-09-02.** Investigation found the backend chain was already built and live on a DIFFERENT front door (`/apply/applicant` + `/apply/agent`, reached via `FinancialsAccessButton`): it already hands off into `/apply?listingApp=<id>`, Stripe checkout already fires `/api/trigger-screening` on `checkout.session.completed` (not board approval — that claim in an earlier draft of this section was wrong, confirmed by reading `app/api/webhooks/stripe/route.ts`), and `app/api/apply/link-listing` already bridges `detailed_application_id` back onto `listing_applications`. The actual gap was that NONE of this was reachable from `/pre-apply/[code]` — the flow staff and applicants actually use daily. Shipped: a `ScreeningPaymentGate` on `app/pre-apply/[code]/page.tsx` — every `role='applicant'` stakeholder (lead AND co-applicants) on a `lease`/`purchase`/`additional_occupant` application now sees the full document checklist (read-only) plus a payment/screening gate instead of upload boxes, until `listing_applications.detailed_application_id` is set; opens the existing `/apply?listingApp=...` wizard in a new tab, "Check again" re-loads once it bridges back. Co-applicants are gated too (not exempt — corrected 2026-09-02: they get their own Checkr check, they just don't have to be the one who pays, since `detailed_application_id` lives on the shared `listing_applications` row and whichever applicant in the group pays first clears it for the rest); owners and agents are never gated (they're never screened). This flow collects no age field on a stakeholder, so a minor can't be told apart from an adult co-applicant/occupant — gating everyone with `role='applicant'` is the conservative default until that's collected. Also fixed a real collision this surfaced: `link-listing` marks every `role='applicant'` stakeholder `'completed'` immediately on payment — correct for the lightweight `/apply/applicant`+`/apply/agent` pattern (paying IS the whole application there) but would have shown the pre-apply applicant "you're all done" before they'd uploaded anything; `DocsStep`'s "already done" check now also requires their own checklist to actually be complete.
2. Checklist-preview screen + the acknowledgment e-sign (steps 1–2, including the 45-day notice) — 🟡 **shipped 2026-09-02.** `ScreeningPaymentGate` previously just listed the checklist inline next to plain warning text; a real e-signed acknowledgment (checkbox + typed name + drawn signature, same inline mechanics as the existing rules acknowledgment further down the same page — no separate emailed link needed, since this is the same already-OTP-verified session) now gates the payment button, covering both the checklist shown and the 45-day deadline. New `application_stakeholders.checklist_ack_*` columns, distinct from the existing rules-ack columns.
3. Board visibility filter (step 7) — 🟡 **shipped 2026-09-02, narrower than the original design.** Investigation found the board queue's LIST (`GET /api/units/pre-apply`, and `getApplicationDashboard`'s `submittedOnly`) already excluded anything without `listing_applications.submitted_at` set — which `lib/preapply.ts`'s `submitIntake()` only sets once every required document has actually arrived, so an unsubmitted/in-progress application was never listed. The real gap was the per-application detail/action endpoint (`GET`/`PATCH /api/units/pre-apply/[id]`, what `/units/applications` actually opens and approves through): it had no matching guard, so a reviewer with a bookmarked or guessed id could still open, and even approve or decline, an application the applicant hadn't finished submitting. Fixed by adding the same `submitted_at` check there. Deliberately does NOT change the existing live workflow where board members/on-site managers review and approve individual documents as they arrive on an already-submitted application (`/board-review/[token]`'s per-document review rounds) — a literal reading of "board only ever sees complete [= every document already approved] applications" would have ended that workflow entirely and moved per-document review to staff-only, which the user did not choose.
4. Screening validity display + 10/5/1-day warning cron + 45-day expiration + $150 re-screening payment — 🟡 **shipped 2026-09-02** (PR #751). `lib/screening/validity.ts` is the single source of truth for the 45-day window; staff and board review pages show a "valid through" date + expired badge; a daily cron warns at 10/5/1 days then, on actual expiry, emails a one-time `/rescreen/[token]` link for the flat $150 re-screening charge, which fires a fresh Checkr order via the same `/api/trigger-screening` endpoint on payment.
5. Lease non-renewal escalation (independent of 1–4). ✅ **Shipped 2026-09-15** — `lib/lease-escalation.ts`, `/admin/lease-escalations`, `/api/cron/lease-escalations`, migration `20260915_lease_renewal_escalation.sql`. The escalation notice, the 15-day violation-fee clock, the staff decision screen and the red blown-window flag are all built; the timeline runs from the lease end (T / T+15 / T+45) per the user's 2026-09-15 reset, not from T-30. The grace-window half shipped earlier (PR #883, `lib/lease-renewal-rule.ts`): a renewal requested > 30 days after the previous lease end opens as a new lease (screening + fee).
...
7. Power of attorney / legal guardian signing — 🟡 **shipped 2026-09-02** (independent of 1–6, builds on top of Phase 6's occupant roster). Real case: Unit 706, an elderly additional resident who can't sign or consent for himself. See the section above for the full design (shipped as specified) and the explicit "never fake it by reusing their email" call.
...
## 🟡 PARTIAL — association onboarding questionnaire: "What they have today" LIVE, run for VPCI (2026-09-11); board assessment still 🔴

Guided setup per association: every answer a timestamped, attributed board decision (append-only `association_onboarding_decisions`), applied to the live config tables only on adoption at a board meeting. Built: identity facts, applications & screening, eligibility rules, document checklist grid, board & approvals, review & adopt. `/admin/cinc-sync/[code]/onboarding`. 🔴 Not built: governing-documents upload → MAIA rule proposals, board-side self-answering page, Rules & Regulations redraft (staff review before publish — user decision), compliance/operations/residents sections. **2026-09-11:** MAIA now reads each association's filed application package / Rules / Declaration (scans transcribed once) and proposes the answers with quoted evidence on the questionnaire's first panel; accepted proposals apply at once as existing configuration (PRs #877, #879). Run for VPCI: 22 proposals, 10 differ from live — awaiting staff review. Both onboarding migrations applied 2026-09-11 (the Sep 9 tables had never actually been created — a head-count check lied). Next: the user uploads the missing packages (TROP, LCLUB) and Rules (KANE, KGA, LCLUB, MACO, SHORE, SP, WBP, WBPA), then run all 25; then build the board assessment page. Application Guide now also live for VPCI (#878). Detail: `docs/SESSION-HANDOFF.md`.
...
- ✅ **Spot-checked 2026-08-31.** `scripts/spot-check-car-registrations.ts` (built this session — no sandbox credentials to run it, so the actual check was done by hand: querying `application_documents` for `doc_key='car_registration'` created before the 2026-08-22 Sonnet switch, then reading each stored file directly) found exactly 2 documents in scope, portfolio-wide: MANXI 613 (Mark Anthony Leguizamon) — stored `2027-03-08` matched the real registration, already correct from the original incident fix. MANXI 912 (Querline Yasmine Pinckney) — stored `2027-11-03` was wrong; corrected to the real printed date, `2028-03-14`. No other units were affected. The script remains available for any future re-scan need (e.g. after a similar model-accuracy fix elsewhere).
...
### 2026-09-07/08 additions (PRs #809–#820) — signer correctness, a real process gate, and board-card visibility

- ✅ **Decider/Voter (Committee) config now respected by the automatic decision letter.** `lib/board-decision-letter.ts`'s `loadDecisionContext()` used to pick signers by board-title priority only (President > VP > Secretary > Treasurer), unaware of the association's "Committee — Application Approval" (`board_approval_members`, Decider vs Voter) config a DIFFERENT older flow already honored. Real incident confirmed via evidence: a configured Voter (Karen Fisher) got a real "Please sign" email; the configured Decider (Walter Leslie Joseph IV Giles) did not. Fixed for both the automatic path and the manual staff decision-page route.
- ✅ **The decision letter now REQUIRES a board review round to have actually gone out first.** Staff approving every document directly (without ever clicking "🏛 Send to the board to review") could previously still trigger the automatic letter — user: "before sending the approval letter Maia should have sent the SEND TO THE BOARD REVIEW - not the letter itself." New `ensureBoardReviewRoundSent()` (`lib/board-review-email.ts`) auto-sends a round if none exists yet and blocks the letter until one does.
- ✅ Board review round email shows the REAL due date + days-left, not the generic "may decide up to N days" sentence, whenever `ensureBoardReviewRoundSent` fires for an application whose window is already open (previously hid that the clock had already started).
- ✅ Staff applications list now surfaces `windowOpenedAt` (PR #815) and **who finalized the checklist and when** (name + role: Staff/Board/On-site manager, via a new `application_document_reviews` lookup) — user had asked twice: "I still can't see in my dashboard when someone approved all files" then "we have the name and timestamp of who approved, don't we have?"
- ✅ **Owner balance + ledger link on the board review card** (PR #817). "board wants to see... the unit owner balance and a link with his ledger, since they won't approve if the unit is not current" — banner (green/current, red/balance-due) on `buildReviewRoundEmail`, fetched live from CINC (`getHomeownerPaymentBlockStatus`) on every send (initial + every reminder resend), so nothing needs a separate daily refresh job. Link is a login-free `signLedgerToken` pointing at the existing `/api/owner/ledger/[token]` live-PDF statement endpoint (same one the owner self-service flow uses).
- ✅ **Per-association board-decision-rule text, quoted verbatim** (PRs #818–#819), new `lib/board-decision-rules.ts` — same precedent as `lib/manxi-rules-ack.ts`/`lib/vpci-rules-ack.ts` (legal text lives in code, reviewed via PR, never a casually-editable DB field or a guessed sentence). **Confirmed for MANXI only** (user supplied the actual "Amendment to Rules and Regulations" PDF — 30 calendar days, no stated consequence for a miss) **and VPCI** (already-extracted `VPCI_INSTRUCTIONS[1]`, Declaration Article XXII — 10 BUSINESS days, deemed approved if missed). Nothing is shown for any other association; this deliberately does NOT assert Florida statute imposes one universal rule (researched, couldn't get verified statute text — web access to flsenate.gov/DBPR/law-firm sources was blocked from this session; secondary sources disagreed with each other).
- ✅ **VPCI's window actually runs on 10 BUSINESS days now, not the 30-calendar-day default** (PR #820) — `board_window_days` was silently 30 for every application including VPCI, so its Declaration's real (and stricter, deemed-approved) clock was never honored by the actual reminder/decision-letter timing. `lib/board-decision-rules.ts`'s `boardWindowOverrideFor` + `addBusinessDays` (Mon–Fri, **no holiday calendar** — none exists in this codebase); `lib/board-review.ts`'s new `computeBoardWindow()` is the one place `dueAt` is derived (used by both `deriveReviewState` and `syncBoardWindow`, which used to duplicate the formula). `dueAt` is always computed fresh, never stored, so this fixed every VPCI application — past and future — with nothing to backfill.
- 🔴 **Open: the other 23 associations.** No confirmed decision-rule text or window-unit override exists for any association besides MANXI/VPCI — every other association still silently assumes the 30-calendar-day default, which may or may not match its actual Declaration. Needs each association's governing-document language (paste the relevant page, or — once this session has live Supabase storage credentials again — extract from `associations.rules_pdf_path` where already on file).
- 🔴 **Open: MANXI's source PDF was never filed.** Only the extracted rule text lives in `lib/board-decision-rules.ts`; the actual "Amendment to Rules and Regulations" PDF was not uploaded to `associations.rules_pdf_path` — this session had no live Supabase storage credentials to do that upload.

## ✅ LIVE — Checkr report auto-filed onto the checklist + a colorful summary spliced INTO the same PDF (PRs #800–#808, #812–#813)

A completed Checkr report used to sit only in `screening_subjects` (viewable from a "Background check" summary card) — never in the applicant's own "Background / Credit Reports" checklist row, so the required document read as permanently missing with none of the usual preview/approve flow.

- ✅ **Auto-file** (`lib/screening/report-storage.ts`'s `fileReportAsDocument`) copies the same fetched PDF bytes into `application_documents` the moment a report completes, scoped to the right applicant via `screening_subjects.stakeholder_id` (set once at order-creation time from the applicant MAIA is actually placing the order for — **not** sent to Checkr's own API as their `custom_id`/equivalent, since that field isn't confirmed in their docs; purely an internal linking column). A single-applicant fast-path plus normalized name-matching cover subjects created before this column existed.
- ✅ **Colorful summary** (`lib/screening/report-summary.ts`'s `summarizeReport()` + `components/ScreeningReportSummary.tsx`) — credit score, credit-file/report status, and real `credit_summary` stats (monthly payment, total accounts, average account age, credit utilization, collections/bankruptcies/charge-offs, on-time-payment trend), parsed from the raw Checkr `report_data` jsonb and verified against a real captured report (Querline Pinckney, MANXI 912: 585 score, "consider"). Shown on the admin pre-apply page, the board review page, AND on the per-applicant checklist row itself (initially missed the checklist-row location, fixed same session). Never silently renders nothing on unparseable data — always shows at least a status chip + a raw-data toggle, which is what surfaced the real JSON shape to build the parser against in the first place.
- ✅ **One merged PDF, not a second button.** First shipped (PR #808) as a SEPARATE downloadable "🖨 Colorful summary (PDF)" button — user corrected: *"my idea was having the new colourful in the same PDF and Preview as the original, so both could be seen by the board while approving, not a new button that I will never use."* Fully reworked (PR #816): `lib/screening-summary-pdf.tsx`'s `mergeWithOriginalPdf()` (pdf-lib) splices the MAIA colorful cover page in FRONT of Checkr's original report, byte-for-byte unmodified — same compliance pattern as `lib/rules-ack-pdf.ts`'s `mergeRulesIntoWrapper` (a regulated document's real pages are copied verbatim, never re-created). One file now behind the existing "View report" link and the existing "📄 File as document" button (which now doubles as `regenerateMergedReport()` — the recovery path for a report completed before this shipped; a report already on file needs that button clicked once to pick up the merged version).
- ✅ **Manual resend of the $150 re-screening payment link** — "don't forget to send the countdown days the same email to them to review" (PR #805), a staff button for cases where the countdown reminder needs re-triggering by hand.
- ✅ **Open applications stuck on a dead Rentvine fallback panel, fixed** (PRs #812–#813) — root cause was `listing_applications.screening_provider` being a frozen per-application snapshot taken before the association-wide flip to `maia_checkr`; `resolveScreeningProvider(raw)` treats `NULL` identically to the literal string `'tenant_evaluation'`, so the first backfill migration's `WHERE screening_provider = 'tenant_evaluation'` silently skipped every NULL row and had zero effect — corrected with `OR screening_provider IS NULL` in both the migration and `lib/migration-status.ts`.
- ✅ **24 duplicate "tenant left a message" notification emails, fixed** (PR #814) — `/api/request/[token]/note/route.ts` had no dedupe (now a no-op when the note is unchanged from the last save) AND the public note form never cleared/disabled after a successful send, so repeated clicks resent the identical text and fired a fresh email every time. Fixed both ends.
...
<details>
<summary>Full history of the Blocker 2 investigation (resolved 2026-09-03)</summary>
...
**2026-08-30/31: found `credit_report`/`eviction_history` both `null`.** Real progress happened in two rounds before landing here:
1. **A real, separate bug was found and fixed first** (worth keeping regardless of the outcome below): `app/api/admin/applications/create-test/route.ts` was pairing a documented canned SSN with the wrong name/DOB in two places (the default applicant and the custom-name override), which — per Checkr's Testing guide — silently falls through to an inert "clean scenario for every product" instead of real canned data. Fixed; all test scenarios now use exact documented tuples.
2. **That fix did not explain the null result.** A live re-run with the *exact* Madelyn Webster tuple (criminal "clear" / credit "consider" per Checkr's own scenario table) still returned `credit_report`/`eviction_history` both `null`, on a report Checkr itself marked complete. Requesting them explicitly via `add_on_products` (Checkr's Orders API docs: an unknown/non-add-on slug returns a validation error) got a `422` from `POST /orders`: `{"errors":[{"code":"validation_error","detail":"Unknown add-on products: credit_report, eviction_history","source":{"pointer":"/add_on_products"}}]}` — misread at the time as final confirmation the account lacked these products entirely.

**2026-09-01: Checkr support (William, Checkr Tenant) replied** that the null-report finding was actually a bug on CHECKR'S side ("a recent change made it so test mode credit reports and eviction checks were not populating correctly"), fixed in production as of that date — contradicting the 2026-08-31 conclusion. Flagged as not yet re-verified on our end at the time.

**2026-09-03: re-verified.** See the resolution note above.
