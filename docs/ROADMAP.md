...
## ⚠️ Checkr — integration re-proven working, ONE blocker before going live (updated 2026-08-31)
...
**Blocker 2 (2026-08-30) — RESOLVED 2026-08-31, was a test-data bug, not a real product gap.** The `credit_report`/`eviction_history: null` finding came from a test order whose applicant used SSN `333-33-3333` (the real, documented Norma Davies canned tuple's SSN) paired with a generic name and the wrong DOB. Per Checkr's own Testing guide (`checkr-tenant-api-docs.redocly.app/testing#canned-provider-scenarios`, and confirmed directly by Checkr support/Victor, 2026-08-31 email) a canned scenario only fires on an **exact** match of first_name + last_name + dob + ssn together — a partial match silently falls through to an inert "clean scenario for every product," not the real canned data for that ssn. Fixed at the root: `app/api/admin/applications/create-test/route.ts` had this same bug in two places (the default applicant AND the custom-name override), both corrected to either use an exact documented tuple or a deliberately non-matching ssn. Also added what was structurally missing before this: MAIA never had a code path that read the structured report body at all (`getReportPdf` only ever fetched the rendered PDF) — `lib/screening/checkr.ts` now has `getReport()` (`GET /reports/{id}`, raw JSON passthrough — not parsed into typed fields yet, since the exact schema hasn't been confirmed against a real live response from this environment), stored in the new `screening_subjects.report_data` column and viewable per-applicant in `/admin/applications`. Two new test scenarios added for verification: **Madelyn Webster** (credit report flagged "consider" — the direct test that credit data comes back populated) and **Ingrid Vance** (bank-sourced income verification). **Still needs a real Test Environment run to fully confirm** — this was fixed by code reading, not by executing against the live sandbox (no Checkr credentials in the environment this was done from).

**Do not set any association's `screening_provider` to `maia_checkr` until Blocker 1 is resolved and the fix above is confirmed live.** A sandbox key returns canned results for real people — worse than an error, because nothing looks wrong. Offered, not built: an admin diagnostic reporting only the key prefix.
