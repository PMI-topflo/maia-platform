...
const SKIP: { category: FilterCategory; re: RegExp; reason: string }[] = [
  ...
  // `full report` / `screening` = the Checkr background/screening report PDF.
  { category: 'criminal', re: /(\b(criminal|background\s*(check|report)|arrest|eviction\s*(report|history)|checkr|clearance|screening)\b|full[\s_]?report)/i, reason: 'looks like a criminal / background / screening report' },
...
const INCLUDE: { category: FilterCategory; re: RegExp; reason: string }[] = [
  ...
  { category: 'approval',           re: /\b(board\s*approval|approval\s*letter|approval|approved|new\s*tenant|new\s*owner|application\s*approv)\b/i, reason: 'board approval letter' },
...
export function filterDriveFile(name: string, path: string | null | undefined, whitelistOn = true, isGoogleDoc = false): FilterDecision {
  ...
  // Underscores → spaces so \b word boundaries match our own renamed files
  // (e.g. "2026_05_Lease.pdf" → "…Lease…", "2026_07_Approval.pdf").
  const hay = `${path ?? ''} / ${name}`.replace(/_/g, ' ')
