...
NOTIFY pgrst, 'reload schema';`,
  },
  {
    ...
    description: 'Adds 3 columns to applications for the documents international applicants provide alongside the plain domestic Checkr check: a foreign police clearance/criminal record, a CPA Financial Certification (replaces an earlier bank-statements + bank-reference-letter design -- one comprehensive CPA-prepared report covering income/assets/liabilities/net worth with USD conversion is clearer for board members than raw bank statements), and a notarized English translation when any of those are in a foreign language. Applicants can upload these from the start of /apply (Documents step), before paying, since some (a foreign police clearance, a CPA report) can take weeks to obtain -- uploads are optional and do not block submission',
...
NOTIFY pgrst, 'reload schema';`,
  ...
  {
    key:         'lease_renewal_prior_approval_letter_label',
    label:       'Lease renewal: "Board Approval Letter" → "Copy of Last Year\'s Approval Letter"',
    description: "MANXI lease_renewal's board_approval_letter item read as a bare \"Board Approval Letter\" — indistinguishable, out of context, from the NEW letter MAIA itself generates once the board approves THIS renewal. It's actually optional supporting documentation: the PRIOR letter from when the tenancy was first approved, if the owner happens to have it. User report, 2026-08-21 (MANXI 110, Susie Bell renewal, owner Monica Blumenfeld): \"let's make it clear when Lease Renewal on the request, change for Copy of Last Year's Approval Letter.\" Relabeled + given the clarifier the `lease` type's own row already has, adapted for \"prior\" rather than \"the\" approval. required=false, provided_by='landlord' and the doc_key itself are unchanged.",
    filename:    '20260821_lease_renewal_prior_approval_letter_label.sql',
    artifact:    { type: 'column', table: 'association_intake_documents', column: 'label' },
    sql: `UPDATE public.association_intake_documents
...
NOTIFY pgrst, 'reload schema';`,
 ...
 WHERE id IN ('e210870d-9658-4f5e-89ef-505e5d4eba98', 'e9da3aca-bcae-4eff-9452-3f75cc315750')
  ...
  {
    key:         'manxi_lease_renewal_affidavit_not_required',
    label:       'MANXI lease renewal: Tenant Affidavit no longer required',
    description: "User report, unit 1002 (Ester Rodao/Julian Osorio renewal): \"Why is asking the Affidavit if it's a lease renewal?\" Found direct, on-the-record confirmation this was already answered once, on this exact unit, 2026-08-01: on-site manager Kaye Brunson (LCAM) told staff \"You are right for lease renewal, the affidavit is not needed again. Please disregard the affidavit for this matter,\" after staff asked \"Why the affidavit again if we have it on the initial application?\" Checked usage since: every tenant_affidavit ever filed on a MANXI lease_renewal application (5, across 9 renewals) came in via uploaded_by_role='drive-pick' or 'staff' — staff backfilling it from the ORIGINAL lease's file, never a fresh applicant/notary response to the renewal's own intake ask. Same shape as the board_approval_letter retirements earlier this session. NOT deactivated (unlike board_approval_letter) — it's a real document type that occasionally IS still missing and worth asking for — just no longer required, so it can't block completion on a renewal the way it did here.",
    filename:    '20260822_manxi_lease_renewal_affidavit_not_required.sql',
    artifact:    { type: 'column', table: 'association_intake_documents', column: 'required' },
    sql: `UPDATE public.association_intake_documents
...
NOTIFY pgrst, 'reload schema';`,
  ...
  {
    key:         'manxi_tenant_affidavit_required_addl_occupant',
    label:       'MANXI: Tenant Affidavit also required for additional occupant',
    description: "User correction on the generated Application Guide PDF, 2026-09-03: \"Tenant Affidavit (signed & notarized by tenant and landlord) - is Req. for additional occupant\" — the guide showed it required only for lease, with additional_occupant blank, because no row existed for that (association_code, application_type, doc_key) combination. Copies the existing MANXI/lease/tenant_affidavit row's label, provided_by, note, sort_order, template_path, requires_notarization, per_applicant and allow_multiple verbatim onto a new additional_occupant row, so the two stay in sync in wording and notarization behavior rather than retyping them by hand.",
    filename:    '20260903b_manxi_tenant_affidavit_required_addl_occupant.sql',
    artifact:    { type: 'column', table: 'association_intake_documents', column: 'required' },
    sql: `INSERT INTO public.association_intake_documents
...
NOTIFY pgrst, 'reload schema';`,
  },
  {
    ...
    description: "User report, 2026-09-08 (Quentin Jamal Smith & Timothy Dean Walker, MANXI Unit 706): \"some open applications still have the Rentvine info and I can't send them to Checkr.\" screening_provider is a snapshot frozen once, at creation, from the association's LIVE value at that moment -- flipping every association's default to maia_checkr (20260906b) only changed what NEW applications snapshot; anything already open from before that flip is stuck reading tenant_evaluation forever, which (since PR #796 retired the manual \"Switch this application to Checkr\" button) now shows a dead-end informational note with no way to trigger Checkr at all. CORRECTED same day, after the first version ran with zero visible effect: resolveScreeningProvider() treats a NULL screening_provider exactly like the literal string 'tenant_evaluation' (its own fallback default), but the first version's WHERE clause only matched the literal string -- SQL's = never matches NULL -- silently skipping every application whose screening_provider had never been populated at all, which was apparently most or all of the stuck ones. One-time backfill: any application still open (not approved/declined/withdrawn) with screening_provider NULL or 'tenant_evaluation' moves onto maia_checkr now; anything already decided is left untouched.",
...
NOTIFY pgrst, 'reload schema';`,
  ...
  {
    key:         'backfill_application_stakeholders_primary_by_role',
    label:       'application_stakeholders — backfill missing primary per role',
    description: "User report, 2026-09-09 (Ashlee Elizabeth Muthra, MANXI Unit 702): the admin dashboard header showed \"Applicant: not set\" even though she was the only, clearly-filled-in applicant in the Applicants card below. Root cause: every \"who is the primary applicant\" read in this codebase (14 call sites, e.g. the header, lib/application-payment-link.ts's sendApplicationPaymentLink, the board-approval handoff) filters .eq('role','applicant').eq('is_primary', true) -- is_primary is scoped PER ROLE by design, not a single whole-application flag. But lib/preapply.ts's addStakeholders() (used whenever someone is added to an application already in progress -- an agent adding the real applicant, mid-intake collaborators, etc.) unconditionally inserted every new person with is_primary: false, regardless of whether their role already had one. When an agent starts the intake (createIntake sets is_primary=true on the AGENT's own row, role='applicant_agent'/'listing_agent'), the real applicant added afterward never became primary -- no role='applicant' row was ever primary, so every one of those 14 reads found nobody. Fixed at the source in the same change (addStakeholders now marks a new person primary within their own role, but only if that role doesn't already have one); this is the one-time catch-up for applications already affected: for every (application_id, role) pair with at least one stakeholder but no row already is_primary=true, marks the earliest-created row of that role primary.",
    filename:    '20260909_backfill_application_stakeholders_primary_by_role.sql',
    artifact:    { type: 'column', table: 'application_stakeholders', column: 'is_primary' },
    sql: `WITH ranked AS (
...
NOTIFY pgrst, 'reload schema';`,
  ...
  {
    key:         'association_onboarding',
    label:       'association_onboarding_sessions + association_onboarding_decisions + board_approval_config.decision_window_days',
    description: "Association onboarding questionnaire (applications scope, 2026-09-09). Two tables: one session row per association (draft → adopted, with the adopting meeting), and an APPEND-ONLY decision register — one row per answer, stamped with who decided (board member name + role, or staff for identity facts), the source (meeting / email consent / staff-confirmed / existing configuration), when, and after adoption when it was applied to the live setting. Nothing reads these at runtime; lib/onboarding.ts applies adopted decisions to the tables that already drive behaviour (associations, association_config, association_application_rules, association_intake_documents, board_approval_config, board_approval_members). Also adds board_approval_config.decision_window_days, read by lib/board-review.ts before the per-application board_window_days column.",
    filename:    '20260909_association_onboarding.sql',
    artifact:    { type: 'table', table: 'association_onboarding_decisions' },
    sql: `-- =====================================================================
...
`,
  ...
  {
    key:         'association_onboarding_proposals',
    label:       'association_onboarding_extractions + association_onboarding_proposals',
    description: "Onboarding questionnaire — \"What they have today\" (2026-09-11). MAIA reads the association's filed application package / Rules / Declaration and proposes the questionnaire's answers with quoted evidence; staff accept or reject on the questionnaire page. Accepted proposals become existing_config decisions and are applied immediately (lib/onboarding-proposals.ts).",
    filename:    '20260911_association_onboarding_proposals.sql',
    artifact:    { type: 'table', table: 'association_onboarding_proposals' },
    sql: `-- =====================================================================
...
`,
  ...
  {
    key:         'application_payment_reconciliation',
    label:       'stripe_payout_receipts + checkr_receipts',
    description: "Accounting → Application payments (2026-09-13): Stripe charges/fees/payouts are read live; these tables hold Karen's 'received in bank' flag per Stripe payout and the Checkr order receipts uploaded from the Checkr dashboard (matched to screening_subjects.checkr_order_id).",
    filename:    '20260913_application_payment_reconciliation.sql',
    artifact:    { type: 'table', table: 'checkr_receipts' },
    sql: `-- =====================================================================
...
`,
  ...
  {
    key:         'associations_board_contact_email',
    label:       'associations.board_contact_email (shared board mailbox)',
    description: "The board's shared mailbox (MANXI: themanorsbuildingxi@gmail.com). Emails that copy the board on something the applicant also receives (interview introduction, invite with board CC) put this address on CC and the members/on-site managers on BCC, so private board emails are never shown to applicants. User direction 2026-09-14.",
    filename:    '20260914_associations_board_contact_email.sql',
    artifact:    { type: 'column', table: 'associations', column: 'board_contact_email' },
    sql: `-- =====================================================================
...
`,
  ...
  {
    key:         'listing_applications_auto_expiry',
    label:       'listing_applications.expiry_notice_* / expired_auto / closed_drive (automatic expiry)',
    description: "Automatic expiry of dead applications (cron application-auto-expiry): open notice (kind/at/due), expired_auto for closes done by MAIA, and closed_drive so Reopen can pull the archived Drive files back. User direction 2026-09-14.",
    filename:    '20260914_listing_applications_auto_expiry.sql',
    artifact:    { type: 'column', table: 'listing_applications', column: 'expiry_due_at' },
    sql: `-- =====================================================================
