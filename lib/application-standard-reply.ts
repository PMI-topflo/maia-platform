...
export async function draftStandardReply(opts: {
  ...
  const [{ data: app }, state] = await Promise.all([
    supabaseAdmin.from('listing_applications').select('id, association_code, unit_label').eq('id', applicationId).maybeSingle(),
  ...
  const uploadRows = outstanding.filter(r => !isEsignItem(r.docKey) && providedByOk(r.providedBy))
  ...
  // surfaced separately below rather than silently disappearing.
  const missingSummary = [...uploadRows, ...formRows].map(label)

  const lines: string[] = []
  lines.push(`Hello${senderName ? ` ${senderName}` : ''},`)
  lines.push('')
  lines.push('Thank you for sending your documents — to make sure everything is correctly filed on your application, please upload them (and anything still outstanding) through your secure link rather than by email:')
  if (uploadLink) {
    lines.push('')
  ...
  if (forms.sent.length) {
    lines.push('')
    lines.push(`We've also sent a separate link${forms.sent.length > 1 ? 's' : ''} to your email to complete and sign: ${forms.sent.map(f => ESIGN_CHECKLIST_ITEMS[f.docKey]?.noun ?? f.docKey).join(', ')}.`)
  }
  if (missingSummary.length) {
    lines.push('')
    lines.push('Still needed:')
    for (const m of missingSummary) lines.push(`  • ${m}`)
  }
  ...
  return {
    ...
    formsSent: forms.sent, formsFailed: forms.failed, missingSummary, nothingOutstanding: false,
